...
def load_if_s3_data(current_year, current_month, dd, is_devnet=False):
    url_market_pp = (
        "https://drift-historical-data.s3.eu-west-1"
        if not is_devnet
        else "https://drift-historical-data.s3.us-east-1"
    )
    url_market_prefix = (
        url_market_pp
        + ".amazonaws.com/program/dRiftyHA39MWEi3m9aunc5MzRF1JYuBsbn6VPcn33UH/market/"
    )
    name_rot = {}
    spot_asts = ["USDC", "SOL", "mSOL", "wBTC", "wETH", "USDT", "jitoSOL"]
    ...
    if dd == "all time":
        ...
        for name in spot_asts:
            ...
            full_if_url = (
                url_market_prefix + name + "/insurance-fund-records/" + str(year) + "/"
            )  # +str(current_month)
            rots = []
            x = current_month
            try:
...
def load_s3_trades_data(markets, START=None, END=None):
    ...
    if START is None:
        START = (datetime.datetime.now(tzInfo) - datetime.timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
    if END is None:
        END = (datetime.datetime.now(tzInfo) + datetime.timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
    dates = [
        x.strftime("%Y%m%d")
...
def load_user_lp(dates, user_key, with_urls=False, is_devnet=False):
    ...
    url_og = (
        url_market_pp
        + ".amazonaws.com/program/dRiftyHA39MWEi3m9aunc5MzRF1JYuBsbn6VPcn33UH/"
    )
        ...
        if data_url not in data_urls:
            ...
            try:
                ...
                if "liquidation" in dd["actionExplanation"].unique():
                    if (
                        user_key
                        in dd[dd["actionExplanation"] == "liquidation"].taker.unique()
                    ):
