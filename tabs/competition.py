...
async def competitions(ch: DriftClient, env):
    ...
    with tabs[3]:
        preset_programs = st.radio(
            "presets:",
            ["drift-competitions", "drift-vaults", "metadaoproject", "custom"],
            horizontal=True,
        )
        g1, g2 = st.columns(2)
    ...
    with tabs[2]:
        txns = await transaction_history_for_account(
            provider.connection, Pubkey.from_string(pid), None, None, 1000
        )
    ...
    with tabs[1]:
        for acts in accounts:
            ...
            if len(res):
                ...
                vault_df = (
                    vault_df.T
                )  # .sort_values(by='next_expiry_ts', ascending=False).T
                st.write(len(res))
                if len(vault_df) < 3:
    ...
    with tabs[0]:
        ...
        s1.metric(
            "number of " + idl["accounts"][0]["name"] + "(s):",
            len(accounts[0]),
            str(len(accounts[1])) + " " + idl["accounts"][1]["name"] + "(s)",
        )
        ...
        if preset_programs == "drift-competitions":
            ...
            with draw_tabs[0]:
                if dochainload:
                    ...
                    try:
                        user_stats_pubkeys = [
                            Pubkey.from_string(x) for x in vault_df["user_stats"].values
                        ]
                        every_user_stats = await ch.program.account[
                            "UserStats"
                        ].fetch_multiple(user_stats_pubkeys)
                        # mode = st.radio('mode:', ['current state', 'extrapolated'], horizontal=True)
                        vault_df[derived_snap_name] = [
                            x.fees.total_fee_paid // 100 for x in every_user_stats
                        ]
                        vault_df["last trade ts"] = [
                            pd.to_datetime(x.last_taker_volume30d_ts * 1e9)
                ...
                pol_vault = (
                    (
                        1
                        - spot.insurance_fund.user_shares
                        / spot.insurance_fund.total_shares
                    )
                    * v_amount
                    / 1e6
                )
                max_prize = np.round(
                    (pol_vault - comp_acct.sponsor_info.min_sponsor_amount / 1e6)
                    * comp_acct.sponsor_info.max_sponsor_fraction
                    / 1e6,
                    2,
                )
                ...
                prizes = [
                    np.round(
                        max(min(ONEK, max_prize / 10), min(max_prize / 25, TENK)), 2
                    ),
                    np.round(
                        max(min(FIVEK, max_prize / 2), min(max_prize / 12, FIFTYK)), 2
                    ),
                    max_prize,
                ]
                ...
                prize_ev = sum(
                    [
                        odds_rounded[i] / sum(odds_rounded) * prizes[i]
                ...
                s3.metric(
                    ...
                    f"${prize_ev:,.2f}",
                    "prize buckets: $" + ", $".join([str(x) for x in prizes]),
                    help="""
    odds of each bucket (out of {"""
                    + str(sum(odds_rounded))
                    + """}):"""
                    + str(odds_rounded)
                    + """
    ...
    """,
                ...
                st.write(
                    "round number:",
                    comp_acct.round_number,
                    " | number of winners:",
                    comp_acct.number_of_winners,
                    " | round end (UTC):",
                ...
                if dochainload:
                    vault_df["entries"] = (
                        vault_df[derived_snap_name]
                        - vault_df["previous_snapshot_score"]
                    ).max(0) + vault_df["bonus_score"]
                    vault_df["entries"] = vault_df["entries"].apply(
                        lambda x: min(x, comp_acct.max_entries_per_competitor)
                    )
                    vault_df["EV ($)"] = (
                        vault_df["entries"] / vault_df["entries"].sum() * prize_ev
                    )
                    vault_df["chance of a win"] = 1 - (
                        (1 - vault_df["entries"] / vault_df["entries"].sum())
                        ** comp_acct.number_of_winners
                    )
                    vault_df[takervolnom] = (
                        vault_df[derived_snap_name]
                        - vault_df["previous_snapshot_score"]
                    ) / 10
                    ...
                    up_to_n_entries = do1.number_input(
                        "entries:", min_value=0, value=1000
                    )

                    direct = do0.radio(
                        "direction:", ["less than", "greater than", "precisely"]
                    )
                    dd = 1
                    dddf = pd.DataFrame([1])
                    if direct == "less than":
                    ...
                    pplnom = (
                        "anyone of " + str(len(dddf)) if direct != "someone" else direct
                    )
                    if chance_of_a_win * 100 > 0.02:
                        do2.write(
                            f"{pplnom} who has {direct} {up_to_n_entries} tickets has a {chance_of_a_win * 100:,.2f}% of {win_type}!"
                        )
                        do2.write(f"odds: 1 in {int(np.ceil(1 / chance_of_a_win))}")

                    else:
                        do2.write(
                            f"{pplnom} who has {direct} {up_to_n_entries} tickets has a {chance_of_a_win * 100:,.9f}% of {win_type}!"
                        )
                        do2.write(f"odds: 1 in {int(np.ceil(1 / chance_of_a_win))}")

                    st.write(
                        "never traded on drift:",
                        (vault_df[derived_snap_name] <= 1e-6).sum(),
                        "/",
                        len(vault_df),
                    )

                    st.write(
                        "never traded this round:",
                        (
                            (
                                vault_df[derived_snap_name]
                                - vault_df["previous_snapshot_score"]
            ...
            with draw_tabs[1]:
                ...
                if "data" in api_result:
                    ...
                    df["prize"] = (
                        df["summaryEvent"].apply(lambda x: float(x["prizeValue"])) / 1e6
                    )
                    df["startDate"] = df["startTs"].apply(
                        lambda x: pd.to_datetime(int(x * 1e9))
                    )
                    df["endDate"] = df["endTs"].apply(
                        lambda x: pd.to_datetime(int(x * 1e9))
                    )
                ...
                winner_txs = [
                    x.strip(" ")
                    for x in """2D2PwCxXAoJSnycuzoCwu2ayER9MkTYnEuCYhe6JxdzLdwtQNcrUrpVwJH1xxrX4jNTnS7Cd6WNhF8DR6fGSTz5c
                ...
                3mDQCC3GRpbnrPHeZHcEpPRuBHg2BaiRFL35A4sxpSMwoAETegE7HSbQDGJxW3aaAHkk3rbpKimJgWiCRnZnRxC4""".strip(
                        ""
                    ).split("\n")
                ]
                parser = EventParser(program.program_id, program.coder)
                ...
                c1, c2 = st.columns(2)
                do_check = c1.radio(
                    "see historical round result:", [None, 0], index=0, horizontal=True
                )
