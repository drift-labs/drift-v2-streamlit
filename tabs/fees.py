...
async def fee_page(ch: DriftClient):
    ...
    st.write(
        "note: all values are presented as percentages on notional (e.g. 0.01 = 1%)"
    )
    ...
    col1.markdown(
        "[Perp Trading](https://github.com/drift-labs/protocol-v2/blob/24704d781748f3266840a3c68a5d4f65e629826c/programs/drift/src/math/fees.rs#L345)"
    )
    ...
    tbl1.dataframe(
        df[["TIER", "taker_fee", "maker_rebate", "referee_discount", "referrer_reward"]]
    )

    col2.markdown(
        "[Spot Trading](https://github.com/drift-labs/protocol-v2/blob/24704d781748f3266840a3c68a5d4f65e629826c/programs/drift/src/math/fees.rs#L381)"
    )
    ...
    liq_fees = [
        (
            spot.liquidator_fee / 1e6,
            spot.if_liquidation_fee / 1e6,
            spot.insurance_fund.total_factor / 1e6,
        )
        for spot in spots
    ]
    ttbl2.dataframe(
