...
async def insurance_fund_page(ch: DriftClient, env):
    ...
    with tabs[1]:
        ...
        for i in range(state.number_of_spot_markets):
            ...
            if spot.insurance_fund.total_factor:
                factor_for_protocol = (
                    1
                    - spot.insurance_fund.user_factor / spot.insurance_fund.total_factor
                )
            else:
            ...
            if name == cur_spot:
                bbs[0].metric(
                    f"{name} (marketIndex={i}) insurance vault balance:",
            ...
            rev_pool_tokens = get_token_amount(
                spot.revenue_pool.scaled_balance, spot, "SpotBalanceType.Deposit()"
            )
            ...
            if v_amount > 0:
                staker_apr = (
                    next_payment * 24 * 365 * (1 - factor_for_protocol)
                ) / v_amount
            else:
                staker_apr = 0

            total_if_value += (
                (v_amount / 10**spot.decimals)
                * spot.historical_oracle_data.last_oracle_price
                / 1e6
            )
            if_values[i] = v_amount / 10**spot.decimals
            if name in name_rot:
                total_month_gain += (
                    (name_rot[name]["amount"].sum())
                    * spot.historical_oracle_data.last_oracle_price
                    / 1e6
                )
            ...
            if name == cur_spot:
                ccs[0].metric(
                    "revenue pool",
                    f"{symbol}{rev_pool_tokens / 10**spot.decimals:,.2f}",
                    f"{symbol}{next_payment / 10**spot.decimals:,.2f} next est. hr payment ("
                    + str(np.round(factor_for_protocol * 100, 2))
                    + "% protocol | "
                    + str(np.round(staker_apr * 100, 2))
                    + "% staker APR)",
                )
                ccs[0].write("")
                ...
                ccs[0].write(
                    f"{name} (marketIndex={i}) time since last settle: {np.round((ts - spot.insurance_fund.last_revenue_settle_ts) / (60 * 60), 2)} hours"
                )
    ...
    with tabs[0]:
        ...
        z2.metric(
            "Total Stakes",
            str(len(all_stakers)),
            str(len(authorities)) + " Unique Stakers",
        )

        # st.write(perp_df.columns)
        perp_if_at_risk = perp_df["market.insurance_claim.quote_max_insurance"].astype(
            float
        )
        perp_if_paid = perp_df["market.insurance_claim.quote_settled_insurance"].astype(
            float
        )

        z3.metric(
            "Remaining Perp Coverage:",
        ...
        if rot is not None:
            apr_df = (
                rot["amount"]
                / rot["insuranceVaultAmountBefore"]
                * 100
                * 365.25
                * 24
                / 2
            ).sort_index()  # .rolling(24).mean()
            ...
            ifperf = (
                (rot["insuranceVaultAmountBefore"] + rot["amount"])
                / rot["totalIfSharesAfter"]
            ).sort_index()
            ifperf.name = "ifShare price"
            st.plotly_chart(apr_fig, True)
            st.plotly_chart(ifperf.plot(log_y=True))
            st.plotly_chart(
                (ifperf.resample("1W").last().pct_change() * 100).plot(kind="bar")
            )
    ...
    with tabs[2]:
        ...
        total_shares = [
            spot_markets[r["market_index"]].insurance_fund.total_shares
            for i, r in stakers.iterrows()
        ]
        precisions = [
            (10 ** spot_markets[r["market_index"]].decimals)
            for i, r in stakers.iterrows()
        ]
        stakers["cost_basis"] /= precisions
        stakers["if_shares"] /= precisions
        stakers["last_withdraw_request_shares"] /= precisions
        stakers["last_withdraw_request_shares"] = stakers[
            "last_withdraw_request_shares"
        ].replace(0, np.nan)
        stakers["last_withdraw_request_value"] /= precisions
        stakers["last_withdraw_request_value"] = stakers[
            "last_withdraw_request_value"
        ].replace(0, np.nan)
        stakers["total_shares"] = total_shares
        stakers["total_shares"] /= precisions
        stakers["available_time_to_withdraw"] = stakers[
            "last_withdraw_request_ts"
        ].apply(lambda x: pd.to_datetime((x + unstaking_period) * 1e9) if x != 0 else x)
        stakers["last_withdraw_request_ts"] = stakers["last_withdraw_request_ts"].apply(
            lambda x: pd.to_datetime(x * 1e9) if x != 0 else x
        )
        ...
        i = dcol1.selectbox(
            "market index:", sorted(list(stakers["market_index"].unique()))
        )

        st.dataframe(
            stakers[stakers.market_index == i][
                [
                    "authority",
                    "market_index",
                    "$ balance",
        ...
        df = pd.concat(
            [
                df,
                pd.DataFrame(
                    [[protocol_balances[i], "protocol", 100 - df["own %"].sum(), i]],
                    index=[0],
                    columns=df.columns,
                ),
            ]
        ...
        dcol1.metric(
            "Total Insurance (Market Index =" + str(i) + ")",
            f"{if_values[i]:,.6f}",
            str(len(df.authority.unique())) + " Unique Stakers",
        )
        stakers_market_index = stakers[stakers.market_index == i]
        stakers_frac = (
            stakers_market_index["$ balance"]
            * stakers_market_index["last_withdraw_request_shares"]
            / stakers_market_index["if_shares"]
        )
        dcol1.metric(
            "Amount in Unstaking Cooldown:",
        ...
        with st.expander("request withdraw ts"):
            tpl = (
                stakers_market_index.replace(0, np.nan)
                .dropna(subset=["last_withdraw_request_ts"])
                .set_index("last_withdraw_request_ts")
            )
            st.plotly_chart(
                tpl["last_withdraw_request_value"].sort_index().cumsum().plot()
            )
        ...
        full_if_url = (
            url_market_prefix + selected + "/insurance-fund-stake-records/2023/3"
        )
        try:
            st.write(full_if_url)
    ...
    with tabs[3]:
        ...
        def flat_file_list_container_to_name(x):
            return str(
                "".join(map(chr, [int(y.strip()) for y in x.split("    ")[1:]]))
            ).strip()
        ...
        if len(perp_df):
            ...
            spot_pools = spot_df["spot_market.name"].apply(
                flat_file_list_container_to_name
            )
            vv = perp_df["market.amm.fee_pool.scaled_balance"].astype(float).tolist()
            vv += (
                spot_df["spot_market.spot_fee_pool.scaled_balance"]
                .astype(float)
                .tolist()
            )
            vv += (
                spot_df["spot_market.revenue_pool.scaled_balance"]
                .astype(float)
                .tolist()[:1]
            )
            ...
            fig = go.Figure(
                go.Sankey(
                    ...
                    link={
                        ...
                        "target": [
                            usdc_rev_pool_idx
                            if x < len(all_pool_indexes) - 2
                            else x + 1
                            for x in all_pool_indexes
                        ],
    ...
    with tabs[4]:
        ...
        for x in [0, 1, 2]:
            rr = requests.get(
                "https://mainnet-beta.api.drift.trade/bankruptcies/?pageIndex="
                + str(x)
                + "&pageSize=1000"
            ).json()
