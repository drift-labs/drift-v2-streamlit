...
def load_realtime_book(market_index):
    ...
    market_to_oracle_map = (
        pd.DataFrame(x["oracles"]).set_index("marketIndex").to_dict()["price"]
    )
    market_to_oracle_map
    ...
    df["oraclePrice"] = None
    df.loc[df.marketType == "perp", "oraclePrice"] = df.loc[
        df.marketType == "perp", "marketIndex"
    ].apply(lambda x: market_to_oracle_map.get(x, 0))

    # todo, invariant may change w/ future spot markets
    df.loc[df.marketType == "spot", "oraclePrice"] = df.loc[
        df.marketType == "spot", "marketIndex"
    ].apply(lambda x: market_to_oracle_map.get(int(x) - 1, 0))

    df1 = df[(df.orderType == "limit")]
    df1.loc[
        ((df1["price"].astype(int) == 0) & (df1["oraclePrice"].astype(int) != 0)),
        "price",
    ] = df1["oraclePrice"].astype(int) + df1["oraclePriceOffset"].astype(int)
...
def get_mm_score_for_snap_slot(df):
    ...
    d["priceRounded"] = d.apply(
        lambda x: rounded_threshold(x["price"], x["direction"]), axis=1
    )
    d["level"] = np.nan
    # print(d)
    ...
    top6bids = (
        d[d.direction == "long"]
        .groupby("priceRounded")
        .sum()
        .sort_index(ascending=False)[["baseAssetAmountLeft"]]
    )
    top6asks = (
        d[d.direction == "short"].groupby("priceRounded").sum()[["baseAssetAmountLeft"]]
    )

    tts = pd.concat(
        [
            top6bids["baseAssetAmountLeft"].reset_index(drop=True),
            top6asks["baseAssetAmountLeft"].reset_index(drop=True),
        ],
        axis=1,
    )
    tts.columns = ["bs", "as"]
    ...
    score_scale = score_scale * pd.Series(
        [2, 0.75, 0.5, 0.4, 0.3, 0.2]
    )  # , .09, .08, .07])
    chars = ["A", "B", "C", "D", "E", "F", "G", "H"]
    for i, x in enumerate(top6bids.index[:6]):
        char = chars[i]
        ba = d.loc[
            (d.priceRounded == x) & (d.direction == "long"), "baseAssetAmountLeft"
        ]
        ba /= ba.sum()
        d.loc[(d.priceRounded == x) & (d.direction == "long"), "score"] = (
            score_scale.values[i] * ba
        )
        d.loc[(d.priceRounded == x) & (d.direction == "long"), "level"] = char + "-bid"
    for i, x in enumerate(top6asks.index[:6]):
        char = chars[i]
        ba = d.loc[
            (d.priceRounded == x) & (d.direction == "short"), "baseAssetAmountLeft"
        ]
        ba /= ba.sum()
        d.loc[(d.priceRounded == x) & (d.direction == "short"), "score"] = (
            score_scale.values[i] * ba
        )
...
def get_mm_stats(df, user, oracle, bbo2):
    ...
    bbo = (
        df.groupby(["direction", "snap_slot"])
        .agg({"price": ["min", "max"]})
        .unstack(0)["price"]
        .swaplevel(axis=1)
    )
    ...
    bbo_user_score = (
        df.groupby("snap_slot")[["score", "baseAssetAmountLeft"]]
        .sum()
        .reindex(all_snap_slots)
        .loc[bbo2.index[0] : bbo2.index[-1]]
    )
    ...
    near_threshold = 0.002  # 20 bps

    try:
        bid_up = (bbo_user["best dlob bid"] - bbo2["best dlob bid"]) / bbo2[
            "best dlob bid"
        ]
        ask_up = (bbo_user["best dlob offer"] - bbo2["best dlob offer"]) / bbo2[
            "best dlob offer"
        ]
        availability_pct = len(bbo_user.dropna()) / len(bbo_user)
        bid_best_pct = (
            (
                (bbo_user["best dlob bid"] - bbo2["best dlob bid"])
                / bbo2["best dlob bid"]
            )
            == 0
        ).mean()
        ...
        offer_best_pct = (
            (
                (bbo_user["best dlob offer"] - bbo2["best dlob offer"])
                / bbo2["best dlob offer"]
            )
            == 0
        ).mean()
        ...
        pd.DataFrame(
            [
                [
                    ...
                    bid_within_best_pct,
                    offer_best_pct,
                    offer_within_best_pct,
                    bbo_user_avg_score / 100,
                    bbo_user_median_size / 100,
                    bbo_user_min_size / 100,
                ]
            ],
            index=[user],
            columns=[
...
async def mm_page(clearing_house: DriftClient):
    st.warning(
        "historical data only. this data source for snapshot is in the process of being migrated"
    )
    ss1, ss2, ss3 = st.columns([3, 1, 1])
    best_default_source = (
        "https://github.com/0xbigz/drift-v2-orderbook-scored/raw/main/data/"
    )
    ...
    ss = json.loads(
        (await clearing_house.program.provider.connection.get_slot()).to_json()
    )["result"]
    tt = json.loads(
        (await clearing_house.program.provider.connection.get_block_time(ss)).to_json()
    )["result"]
    ...
    def slot_to_timestamp_est(slot, ts_for_latest_slot=None, latest_slot=None):
        ...
        if ts_for_latest_slot is not None:
            return ts_for_latest_slot - (latest_slot - slot) * 0.484

        return TS1 - (SLOT1 - slot) * 0.484
        return 1681152470 - (187674632 - slot) * 0.484
    ...
    tt = market_type + str(market_index)
    df_full = get_data_by_market_index(
        market_type, market_index, source if "data" in source else ""
    )
    ...
    week_ago_slot = df_full[
        df_full.snap_slot > newest_slot - (2 * 60 * 60 * 24 * 7)
    ].snap_slot.min()
    ...
    with tabs[0]:
        ...
        do2 = do2.write(
            "base size=" + str((quote_trade_size / oracle).max().max().round(4))
        )
    ...
    def wm(x):
        ...
        if len(partial_fill_index):
            ...
            full_fills.loc[partial_fill_index] = (
                remainder / weights.loc[partial_fill_index]
            )
        elif remainder != 0:
            full_fills *= np.nan
    ...
    range_selected = molselect.selectbox(
        "range select:", ["daily", "range", "weekly", "last month"], 0
    )
    if range_selected == "daily":
        lastest_date = pd.to_datetime(
            slot_to_timestamp_est(latest_slot_full, ts_for_latest_slot, latest_slot)
            * 1e9,
            utc=True,
        )
        ...
        start_slot = mol2.number_input(
            "start slot:", oldest_slot, newest_slot, oldest_slot
        )
        ...
        mol2.write(
            ...
            + str(
                list(
                    pd.to_datetime(
                        [
                            slot_to_timestamp_est(x, ts_for_latest_slot, latest_slot)
                            * 1e9
                            for x in values
                        ]
                    )
        ...
        values = mol2.slider(
            ...
            int(week_ago_slot),
            int(newest_slot),
            (int(week_ago_slot), int(newest_slot)),
        )
        mol2.write(
            "approx date range: "
            + str(
        ...
        values = mol2.slider(
            ...
            int(oldest_slot),
            int(newest_slot),
            (int(oldest_slot), int(newest_slot)),
        )
        mol2.write(
            "approx date range: "
            + str(
    ...
    bbo2 = pd.concat(
        [lpwm, lmax, df.groupby("snap_slot")["oraclePrice"].max(), smin, spwm], axis=1
    )
    bbo2.columns = [
        "short fill",
        "best dlob bid",
    ...
    st.text(
        "stats last updated at slot: "
        + str(bbo2.index[-1])
        + " (approx. "
        + str(
            pd.to_datetime(
                slot_to_timestamp_est(last_slot_update, ts_for_latest_slot, latest_slot)
                * 1e9
            )
        )
        + ")"
    ...
    st.markdown(
        "[data source](https://github.com/0xbigz/drift-v2-orderbook-snap)"
        + " ([start slot="
        + str(bbo2snippet.index[0])
        + "](https://github.com/0xbigz/drift-v2-orderbook-snap/blob/main/"
        + tt
        + "/orderbook_slot_"
        + str(bbo2snippet.index[0])
        + ".csv))"
        + " ([end slot="
        + str(bbo2snippet.index[-1])
        + "](https://github.com/0xbigz/drift-v2-orderbook-snap/blob/main/"
        + tt
        + "/orderbook_slot_"
        + str(bbo2snippet.index[-1])
    ...
    with tabs[0]:
        ...
        plot1.plotly_chart(
            bbo2snippet.plot(title=market_type + " market index=" + str(market_index))
        )
        df1 = (
            pd.concat(
                {
                    "buy offset + impact": (
                        bbo2snippet["long fill"] - bbo2snippet["oracle"]
                    )
                    / bbo2snippet["oracle"],
                    "buy impact": (
                        bbo2snippet["long fill"] - bbo2snippet["best dlob offer"]
                    )
                    / bbo2snippet["best dlob offer"],
                    "sell impact": (
                        bbo2snippet["short fill"] - bbo2snippet["best dlob bid"]
                    )
                    / bbo2snippet["best dlob bid"],
                    "sell offset + impact": (
                        bbo2snippet["short fill"] - bbo2snippet["oracle"]
                    )
                    / bbo2snippet["oracle"],
                },
                axis=1,
        ...
        buy_pct_within = ((df1["buy impact"] < threshold / 100) * 1).sum() / len(
            df1["buy impact"]
        )
        sell_pct_within = ((df1["sell impact"] > -threshold / 100) * 1).sum() / len(
            df1["sell impact"]
        )
        both_pct_within = (
            (
                (df1["buy impact"] < threshold / 100)
                & (df1["sell impact"] > -threshold / 100)
            )
            * 1
        ).sum() / len(df1["buy impact"])

        buy_pct_within2 = (
            (df1["buy offset + impact"] < threshold / 100) * 1
        ).sum() / len(df1["buy offset + impact"])
        sell_pct_within2 = (
            (df1["sell offset + impact"] > -threshold / 100) * 1
        ).sum() / len(df1["sell offset + impact"])
        both_pct_within2 = (
            (
                (df1["buy offset + impact"] < threshold / 100)
                & (df1["sell offset + impact"] > -threshold / 100)
            )
            * 1
        ).sum() / len(df1["sell offset + impact"])

        fig = df1[["buy impact", "sell impact"]].plot(
            title="perp market index=" + str(market_index)
        )
        fig.update_layout(
        ...
        fig.add_hline(
            y=threshold / 100, line_width=3, line_dash="dash", line_color="green"
        )
        fig.add_hline(
            y=-threshold / 100, line_width=3, line_dash="dash", line_color="green"
        )
        ...
        summarytxt.metric(
            "fill vs oracle within threshold",
            " " + str(np.round(both_pct_within2 * 100, 1)) + " % of time",
            "buys="
            + str(np.round(buy_pct_within2 * 100, 1))
            + "%, sells="
            + str(np.round(sell_pct_within2 * 100, 1))
            + "%",
        )

        summarytxt2.metric(
            "price impact within threshold",
            " " + str(np.round(both_pct_within * 100, 1)) + " % of time",
            "buys="
            + str(np.round(buy_pct_within * 100, 1))
            + "%, sells="
            + str(np.round(sell_pct_within * 100, 1))
            + "%",
        )
    ...
    with tabs[1]:
        ...
        use_ts_est = s2.selectbox(
            "index:",
            ["snap_slot", "est_utc_timestamp"],
            key="tab1",
            help='est utc uses "slot ref:" and "unix_timestamp_ref" to deduce timestamps',
        )
        for user in users:
            ...
            score_emas[str(user)] = (
                bbo_user["score"]
                .fillna(0)
                .rolling(rolling_window, min_periods=min(rolling_window, 1))
                .mean()
            )
            all_stats.append(bbo_user_stats)

        all_stats_df = pd.concat(all_stats, axis=1).T.sort_values(
        ...
        if use_ts_est == "est_utc_timestamp":
            dat.index = [
                pd.to_datetime(
                    slot_to_timestamp_est(x, ts_for_latest_slot, latest_slot) * 1e9,
                    utc=True,
                )
                for x in dat.index
            ]
    ...
    with tabs[2]:
        ...
        o0.write(
            "look up your user account address using your authority + subaccount number"
        )
        authority_l = o1.text_input("wallet address")
        sub_id_l = o2.number_input("subaccount id")
        o3.write(
            get_user_account_public_key(
                clearing_house.program_id, authority_l, sub_id_l
            )
        )
        ...
        user = s1.selectbox(
            ...
            all_users,
            0,
            help="maker address is drift user account address (not wallet address)",
        )
        use_ts_est = s2.selectbox(
            "index:",
            ["snap_slot", "est_utc_timestamp"],
        ...
        st.plotly_chart(
            dat.plot(title=market_type + " market index=" + str(market_index))
        )
    ...
    with tabs[3]:
        ...
        slot = sol1.select_slider(
            "individual snapshot", df.snap_slot.unique().tolist(), df.snap_slot.max()
        )
        ...
        bids1 = (
            toshow_snap[toshow_snap.direction == "long"]
            .groupby("price")
            .sum()
        ...
        asks1 = (
            toshow_snap[toshow_snap.direction == "short"]
            .groupby("price")
            .sum()
        ...
        oh = (
            pd.concat({"bid scores": bscores, "ask scores": ascores}, axis=1)
            .sort_index()
            .replace(0, np.nan)
            .dropna(how="all")
        )
        ...
        ddd = (
            pd.concat({"bids": bids, "asks": asks}, axis=1)
            .sort_index()
            .loc[markprice * (1 - slippage / 100) : markprice * (1 + slippage / 100)]
            .replace(0, np.nan)
        )
        ...
        if score_color:
            toshow_snap = toshow_snap.replace(0, np.nan).dropna(
                subset=["score"], axis=0
            )

        st.dataframe(
            toshow_snap.style.applymap(
                cooling_highlight, subset=["direction"]
    ...
    with tabs[4]:
        ...
        is_live = liveness_switch.radio(
            "liveness", ["on", "off"], index=1, horizontal=True
        )
        ...
        def count_down(ts, live=False):
            def do_it():
                ...
                st.table(
                    toshow.groupby("user")["score"].sum().sort_values(ascending=False)
                )
                st.dataframe(
                    toshow.style.applymap(
                        cooling_highlight, subset=["direction"]
                    ).applymap(level_highlight, subset=["level"]),
                    use_container_width=True,
                )
    ...
    with tabs[6]:
        ...
        acol = 6
        hist_df = pd.read_csv(
            "https://raw.githubusercontent.com/drift-labs/transaction-tables/master/mm_rewards.csv",
            parse_dates=[dcol],
            index_col=[dcol],
        ).sort_index()
