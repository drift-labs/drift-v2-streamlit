...
def get_mm_stats(df, user, oracle, bbo2):
    ...
    bbo_user_score = (
        df.groupby("slot")[["score", "baseAssetAmountLeft"]]
        .sum()
        .reindex(all_slots)
...
async def mm_program_page(clearing_house: DriftClient, env):
    ...
    st.write(
        "subject to change liquidity score component of maker scores. these numbers are relative standings per market. final score are calculated over larger interval and aggregated across markets."
    )
    tabs = st.tabs(
        ["mm leaderboard", "mm individual", "individual snapshot", "settings"]
    )
    ...
    url_agg = (
        source
        + "mainnet-beta/aggregate-liq-score/"
        + market_type
        + "-"
        + str(market_index)
        + ".csv.gz"
    ...
    total_agg_df["approx_date"] = [
        pd.to_datetime(
            slot_to_timestamp_est(x, ts_for_latest_slot, latest_slot) * 1e9, utc=True
        )
    ...
    week_ago_slot = total_agg_df[
        total_agg_df.slot > newest_slot - (2 * 60 * 60 * 24 * 7)
    ].slot.min()
    ...
    lastest_date = pd.to_datetime(
        slot_to_timestamp_est(latest_slot_full, ts_for_latest_slot, latest_slot) * 1e9,
        utc=True,
    )
    date = lastest_date
    date_range = None
    ...
    if date_range is not None:
        mol2.write(
            f"approx date range: `{date_range[0].strftime('%Y-%m-%d %H:%M:%S')}` to `{date_range[1].strftime('%Y-%m-%d %H:%M:%S')}`"
        )

    total_agg_df = total_agg_df[
        (total_agg_df.slot >= values[0]) & (total_agg_df.slot <= values[1])
    ]
    ...
    st.markdown(
        f"above dataframe start slot: [{(smallest_slot)}](https://explorer.solana.com/block/{(smallest_slot)}) (approx: {approx_start_date})"
    )
    st.markdown(
        f"above dataframe end slot:   [{(largest_slot)}](https://explorer.solana.com/block/{(latest_slot)}) (approx: {approx_end_date})"
    )

    date_strr = date.strftime("%Y-%m-%d")
    df_full, ffs = get_data_by_market_index(
        market_type,
        market_index,
        env,
        date_strr,
    ...
    if df_full.currentSlot.max() < values[1]:
        st.write(
            "missing slots from desired end:", values[1] - df_full.currentSlot.max()
        )
    ...
    bbo = (
        df.groupby(["direction", "currentSlot"])
        .agg(
    ...
    with tabs[0]:
        ...
        # st.write(piv_agg)
        all_users2 = (
            df.groupby("user")["score"].sum().sort_values(ascending=False).index
        )
        ...
        metmet1.metric(
            "total avg score:",
            np.round(piv_agg.sum(axis=1).mean(), 2),
            help="this is relative standing normalized across slot",
        )
        ...
        echart.plotly_chart(
            dat.rolling(rolling_window, min_periods=1).mean().plot(),
    ...
    with tabs[1]:
        ...
        user = s1.selectbox(
            ...
            all_users,
            0,
            help="maker address is drift user account address (not wallet address). You can use the above to find your account address.",
        )
        use_ts_est = s2.selectbox(
            "index:",
            ["slot", "est_utc_timestamp"],
    ...
    with tabs[2]:
        ...
        slot = sol1.select_slider(
            "individual snapshot", df.slot.unique().tolist(), df.slot.max()
        )
        ...
        snapshot_type = dol1.radio(
            "snapshot_type:", ["raw", "scored"], index=1, horizontal=True
        )
        snapshot_display = dol2.radio(
            "snapshot_display:", ["raw", "filtered"], index=1, horizontal=True
        )
        ...
        if not toshow_snap.empty:
            ...
            ddd = (
                ...
                .sort_index()
                .loc[
                    markprice * (1 - slippage / 100) : markprice * (1 + slippage / 100)
                ]
                .replace(0, np.nan)
            )
