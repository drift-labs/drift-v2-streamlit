...
async def show_network(clearing_house: DriftClient):
    ...
    dfs = dfs[
        dfs.taker.isin(
            dfs.groupby("taker").count().iloc[:, 0].sort_values().index.tolist()[-10:]
        )
    ]
    if len(dfs):
        ...
        with col4:
            source = st.selectbox(
                ...
                ["taker", "maker", "filler", "actionExplanation"],
            )
        with col5:
            target = st.selectbox(
                'Choose "target" column',
                ["maker", "taker", "filler", "actionExplanation"],
            )
        with col6:
            weight = st.selectbox(
        ...
        for node, adjacencies in enumerate(G.adjacency()):
            node_trace["marker"]["color"] += tuple(
                [len(adjacencies[1])]
            )  # Coloring each node based on the number of connections
            node_info = (
