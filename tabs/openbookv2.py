...
async def tab_openbookv2(ch, env):
    ...
    with tabs[3]:
        preset_programs = st.radio(
            "presets:", ["openbookv2", "metadaoproject", "custom"], horizontal=True
        )
        g1, g2 = st.columns(2)
    ...
    with tabs[2]:
        ...
        txns = await transaction_history_for_account(
            provider.connection, Pubkey.from_string(addysource), None, None, targetNum
        )
        ...
        if len(df):
            ...
            if do_check:
                ...
                for idx in idxes:
                    ...
                    transaction_got = await provider.connection.get_transaction(
