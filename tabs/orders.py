...
async def slot_ts_fetch(_ch):
    ...
    tt = json.loads(
        (await _ch.program.provider.connection.get_block_time(ss)).to_json()
    )["result"]
    st.write(ss, tt)
    return (ss, tt)
...
@st.cache_data
def cached_get_orders_data(
    _ch: DriftClient, depth_slide, market_type, market_index, order_filter
):
    loop = asyncio.new_event_loop()
    return loop.run_until_complete(
        get_orders_data(_ch, depth_slide, market_type, market_index, order_filter)
    )
...
async def get_orders_data(
    ...
    all_users = await _ch.program.account["User"].all(
        filters=[MemcmpOpts(offset=4352, bytes="2")]
    )
    ...
    mm = (
        ...
        if market_type == "perp"
        else state.number_of_spot_markets
    )
    for perp_idx in range(mm):
        ...
        for x in all_users:
            ...
            for order in orders:
                ...
                order_market_type = (
                    str(order.market_type).split(".")[-1].replace("()", "").lower()
                )
                if (
                    str(order.status) == "OrderStatus.Open()"
                    and order.market_index == perp_idx
                    and order_market_type == market_type
                ):
                    ...
                    if order.price != 0:
                        ...
                        order_type_dict[order_type] = order_type_dict.get(
                            order_type, []
                        ) + [order]
        ...
        d_longs_order_type = [
            str(order.order_type).split(".")[-1].split("()")[0] for order in longs
        ]
        d_longs_order_type = [
            x + " [POST]" if longs[idx].post_only else x
            for idx, x in enumerate(d_longs_order_type)
        ]
        d_longs_order_type = [
            "Oracle" + x if longs[idx].oracle_price_offset != 0 else x
            for idx, x in enumerate(d_longs_order_type)
        ]
        ...
        d_shorts_order_type = [
            str(order.order_type).split(".")[-1].split("()")[0] for order in shorts
        ]
        d_shorts_order_type = [
            x + " [POST]" if shorts[idx].post_only else x
            for idx, x in enumerate(d_shorts_order_type)
        ]
        d_shorts_order_type = [
            "Oracle" + x if shorts[idx].oracle_price_offset != 0 else x
            for idx, x in enumerate(d_shorts_order_type)
        ]
        ...
        price_min = float(oracle_price / 1e6) * float(1 - depth_slide * 0.001)
        price_max = float(oracle_price / 1e6) * float(1 + depth_slide * 0.001)
        drift_depth = pd.DataFrame(columns=["bids", "asks"])
        drift_order_depth = pd.DataFrame(columns=["bids", "asks"])
        if market_type == "perp":
            drift_order_depth, drift_depth = calc_drift_depth(
                oracle_price / 1e6,
                market.amm.long_spread / 1e6,
                market.amm.short_spread / 1e6,
                market.amm.base_asset_reserve / 1e9,
                price_max,
                order_data,
            )
        else:
            dd = requests.get(
                "https://openserum.io/api/serum/market/8BnEgHoWFysVcuFFX7QztDmzuH8r5ZFvyP3sYwn1XTh6/depth"
            ).json()
            lenn = 13
            bb = (
                pd.DataFrame(dd["bids"])
                .set_index(0)[[2]]
                .iloc[-lenn:]
            ...
            drift_order_bids = (
                pd.DataFrame(
                    order_data["bids (price, size)"], columns=["price", "bids"]
                )
                .set_index("price")
                .dropna()
            )
            ...
            if order_data["asks (price, size)"] != [""]:
                drift_order_asks = (
                    pd.DataFrame(
                        order_data["asks (price, size)"], columns=["price", "asks"]
                    )
                    .set_index("price")
                    .dropna()
                )
                drift_order_asks["asks"] = (
                    drift_order_asks["asks"].astype(float).cumsum()
                )
                drift_order_asks.rename_axis(None, inplace=True)
                drift_order_asks = drift_order_asks.loc[:price_max]

            drift_order_depth = (
                pd.concat([drift_order_bids, drift_order_asks])
                .replace(0, np.nan)
                .sort_index()
            )
            _idxs = []
            ...
            ss = (
                [drift_depth.index.min()]
                + [
                    x
                    for x in drift_order_depth.index.to_list()
                    if x > drift_depth.index.min()
                ]
                + [drift_depth.index.max()]
            )
            drift_order_depth = drift_order_depth.reindex(
                ss, method="ffill"
            ).sort_index()
...
def calc_drift_depth(
    mark_price, l_spr, s_spr, base_asset_reserve, price_max, order_data
):
    def calc_slip(x):
    ...
    drift_order_bids = (
        pd.DataFrame(order_data["bids (price, size)"], columns=["price", "bids"])
        .set_index("price")
        .dropna()
    )
    drift_order_bids["bids"] = drift_order_bids["bids"].astype(float).cumsum()
    ...
    if order_data["asks (price, size)"] != []:
        # print(order_data)
        drift_order_asks = (
            pd.DataFrame(order_data["asks (price, size)"], columns=["price", "asks"])
            .set_index("price")
            .dropna()
        )
    ...
    drift_order_depth = (
        pd.concat([drift_order_bids, drift_order_asks]).replace(0, np.nan).sort_index()
    )
    _idxs = []
    for v in drift_order_depth.index:
        # add a small value
    ...
    ss = (
        [drift_depth.index.min()]
        + [x for x in drift_order_depth.index.to_list() if x > drift_depth.index.min()]
        + [drift_depth.index.max()]
    )
...
def orders_rpc_page(ch: DriftClient):
    ...
    order_filter = s3.radio(
        "order filter:", ["Limit", "Trigger", "All"], horizontal=True
    )
    ...
    data, oracle_data, drift_order_depth, drift_depth = cached_get_orders_data(
        ch, depth_slide, market_type, market_index, order_filter
    )
    # if len(data):
    ...
    zol2.metric(
        "Oracle Price",
        f"${oracle_data.data.price / PRICE_PRECISION}",
        f"±{oracle_data.data.confidence / PRICE_PRECISION} (slot={oracle_data.slot})",
        delta_color="off",
    )
    ...
    with tabs[0]:
        ...
        bids_quote = np.round(
            df["bids (price, size)"]
            .apply(lambda x: x[0] * x[1] if x != "" else 0)
            .sum(),
            2,
        )
        bids_base = np.round(
            df["bids (price, size)"].apply(lambda x: x[1] if x != "" else 0).sum(), 2
        )
        ...
        asks_base = np.round(
            df["asks (price, size)"].apply(lambda x: x[1] if x != "" else 0).sum(), 2
        )
    ...
    with tabs[1]:
        ...
        fig.add_trace(
            go.Scatter(
                x=drift_depth.index, y=drift_depth["bids"], name="bids", fill="tozeroy"
            ),
            row=1,
            col=1,
        )
        fig.add_trace(
            go.Scatter(
                x=drift_depth.index, y=drift_depth["asks"], name="asks", fill="tozeroy"
            ),
            row=1,
            col=1,
        )
        fig.add_vline(
            x=oracle_data.data.price / PRICE_PRECISION,
            line_width=3,
            line_dash="dot",
            line_color="blue",
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=drift_order_depth.index,
                y=drift_order_depth["bids"],
                name="bids",
                fill="tozeroy",
            ),
            row=2,
    ...
    with tabs[2]:
        ...
        if len(price_df):
            ...
            odf["tradePrice"] = (
                (odf["quoteAssetAmountFilled"].astype(float))
                / odf["baseAssetAmountFilled"].astype(float)
                * 1e3
            )
            odf["oraclePrice"] = odf["oraclePrice"].astype(float) / 1e6
            odf["baseAssetAmountFilled"] = (
                odf["baseAssetAmountFilled"].astype(float) / 1e9
            )
            can_cols = [
                "oraclePrice",
                "tradePrice",
            ...
            def highlight_survived(s):
                return (
                    ...
                    if s.takerOrderDirection == "long"
                    else ["background-color: pink"] * len(s)
                )
            ...
            col3.dataframe(
                tbl.style.apply(highlight_survived, axis=1), use_container_width=True
            )
...
def orders_page(ch: DriftClient):
    ...
    data_source = st.radio(
        horizontal=True,
        label="Data Source:",
        options=[None, "SERVER-BATCHL2", "SERVER-L3", "RPC"],
    )
    ...
    if data_source == "SERVER-BATCHL2":
        ...
        perp_df["bids"] = perp_df["bids"].apply(
            lambda x: float(x[0]["price"]) / PRICE_PRECISION
        )
        perp_df["asks"] = perp_df["asks"].apply(
            lambda x: float(x[0]["price"]) / PRICE_PRECISION
        )
        perp_df["oracle"] = perp_df["oracle"].astype(float) / PRICE_PRECISION
        perp_df["spread"] = (
            (perp_df["asks"] - perp_df["bids"])
            / ((perp_df["bids"] + perp_df["asks"]) / 2)
        ) * 100
    ...
    if data_source == "SERVER-L3":
        ...
        cumulative_bid_depth = [
            sum(bid["size"] for bid in bids[: i + 1]) for i in range(len(bids))
        ]
        cumulative_ask_depth = [
            sum(ask["size"] for ask in asks[: i + 1]) for i in range(len(asks))
        ]

        # Extracting price, size, and maker information for hover data
        bid_hover_text = [
            f"Price: {bid['price']}, Cumulative Size: {cumulative_bid_depth[i]}, Maker: {bid['maker']}"
            for i, bid in enumerate(bids)
        ]
        ask_hover_text = [
            f"Price: {ask['price']}, Cumulative Size: {cumulative_ask_depth[i]}, Maker: {ask['maker']}"
        ...
        if zoom_percentage is not None:
            fig.update_xaxes(
                range=[
                    mid_price * (1 - zoom_percentage / 100),
                    mid_price * (1 + zoom_percentage / 100),
                ]
            )
