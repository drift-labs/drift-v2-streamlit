...
async def perp_lp_page(ch: DriftClient, env):
    ...
    user_lookup = a33.radio(
        "lookup LP users:", ["Active", "DLP", "Ever", "Never"], index=3, horizontal=True
    )
    user_accounts_w_lp = []
    if user_lookup != "Never":
        if user_lookup == "Ever":
            ...
            all_users = await ch.program.account["User"].all(
                filters=[
                    MemcmpOpts(offset=72, bytes="7Ev1Wb17tTZuQFYjieDAx2pbrSzym2eaV")
                ]
            )
        else:
            all_users = await ch.program.account["User"].all(
                filters=[MemcmpOpts(offset=4267, bytes="2i")]
            )
        # with tabs[0]:
    ...
    def calculate_market_open_bids_asks(
    ...
    max_bids, max_asks = calculate_market_open_bids_asks(
    ...
    with tabs[0]:
        ...
        container.write(
            f"lp cooldown time: {state.lp_cooldown_time} seconds | jit intensity: {perp_market.amm.amm_jit_intensity}"
        )
        ...
        if perp_market.amm.amm_jit_intensity > 100:
            ...
            baa_w_amm = perp_market.amm.base_asset_amount_with_amm / 1e9
            protocol_owned_min_side_liq = (
                min(max_asks, max_bids)
            ...
            st.write(
                f"> is baa_w_amm: {baa_w_amm} < {protocol_owned_min_side_liq / 10} ?"
            )
        else:
            st.write(
                f"no lp jit making: amm_jit_intensity={perp_market.amm.amm_jit_intensity} (<=100)"
            )
    ...
    with tabs[1]:
        ...
        if len(user_accounts_w_lp):
            ...
            user_pubkey = ucol.text_input(
                "user account:", "4kojmr5Xbgrfgg5db9bdEuVrDtb1kjBCcNGHsv8S2TdZ"
            )
        ...
        if user_pubkey is not None and user_pubkey.strip() != "":
            ...
            if len(df):
                ...
                df["px"] = -df["deltaQuoteAssetAmount"] / df[
                ...
                for mi1 in df.marketIndex.unique():
                    ...
                    findf = pd.concat(
                        {"cumPnl": s1, "px": s2, "cumBase": s3, "cumQuote": s4}, axis=1
                    )
            ...
            if ff1 != "" and ff2 != "":
                ...
                oraclePx = (
                    pd.DataFrame(oraclePx).iloc[:, 0].apply(lambda x: float(x[1:]))
                )
                ...
                ang["curShares"] = (
                    (
                        ((ang["nShares"] * 10).apply(np.floor) / 10)
                        * ang["action"].apply(
                            lambda x: {
                                "addLiquidity": 1,
                                "removeLiquidity": -1,
                ...
                dres_c["burnCost"] = -(
                    dres_c["myBAA"].abs() * dres_c["SOL-PERP Oracle Price"]
                )
    ...
    with tabs[2]:
        ...
        current_lp_faction_of_amm = (
            perp_market.amm.user_lp_shares / perp_market.amm.sqrt_k
        )
        ...
        lp_fees = (
            fees_by_action.loc["orderFilledWithAmm"] * 0.8 * current_lp_faction_of_amm
        )
    ...
    with tabs[3]:
        ...
        retained_lb = (
            (df["liquidation"] * 0.5).sum()
            + (df["orderFilledWithAmm"].sum() * 0.2)
            + (df["orderFilledWithAmmJit"].sum() * 1)
            + (df["orderFilledWithMatch"].sum() * 0.5)
        )
