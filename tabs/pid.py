...
async def show_pid_positions(clearing_house: DriftClient):
    ...
    see_user_breakdown = col1.radio(
        "see users breakdown:",
        ["All", "Active", "OpenOrder", "SuperStakeSOL", "SuperStakeJitoSOL", None],
        5,
    )
    ...
    if see_user_breakdown is not None:
        try:
            if see_user_breakdown == "All":
                ...
                all_users = await ch.program.account["User"].all(
                    filters=[MemcmpOpts(offset=4350, bytes="1")]
                )
                ...
                all_users = await ch.program.account["User"].all(
                    filters=[MemcmpOpts(offset=72, bytes="3LRfP5UkK8aDLdDMsJS3D")]
                )
            elif see_user_breakdown == "SuperStakeJitoSOL":
                all_users = await ch.program.account["User"].all(
                    filters=[MemcmpOpts(offset=72, bytes="GHB8xrCziYmaX9fbpnLFAMBVby")]
                )
    ...
    if all_users is not None:
        ...
        user_account_pk = get_user_account_public_key(
            clearing_house.program_id, fuser.authority, fuser.sub_account_id
        )
        chu = DriftUser(
            ch,
            user_account_pk,
        ...
        for x in all_users:
            ...
            user_account_pk = get_user_account_public_key(
                clearing_house.program_id, account.authority, account.sub_account_id
            )
            ...
            for idx, pos in enumerate(x.account.perp_positions):
                ...
                if pos.base_asset_amount != 0:
                    ...
                    try:
                        oracle_price = (
                            chu.get_oracle_data_for_perp_market(perp_market)
                    ...
                    if liq_price is None:
                        ...
                        perp_liq_prices[pos.market_index] = perp_liq_prices.get(
                            pos.market_index, []
                        ) + [liq_price]
                        liq_delta = liq_price - oracle_price
                        perp_liq_deltas[pos.market_index] = perp_liq_deltas.get(
                            pos.market_index, []
                        ) + [(liq_delta, abs(pos.base_asset_amount), leverage / 10_000)]
            ...
            for idx, pos in enumerate(x.account.spot_positions):
                ...
                if pos.scaled_balance != 0:
                    ...
                    try:
                        oracle_price = (
                            chu.get_oracle_data_for_spot_market(spot_market)
                        ).price / PRICE_PRECISION
                    except:
                        oracle_price = (
                    ...
                    if liq_price is None:
                        ...
                        spot_liq_prices[pos.market_index] = spot_liq_prices.get(
                            pos.market_index, []
                        ) + [liq_price]
                        liq_delta = liq_price - oracle_price
                else:
        ...
        user_leaderboard = (
            ...
            .sort_values("settled_perp_pnl", ascending=False)
    ...
    col3.metric(
        "Unique Driftoors",
        str((state.number_of_authorities)),
        str((state.number_of_sub_accounts)) + " SubAccounts",
    )
    ...
    with cat_tabs[0]:
        ...
        p1.metric(
            ...
            f"${po_position_notional:,.2f}",
            f"${est_funding_dol * ann_factor:,.2f} interest per year",
        )
        p1.metric(
            "protocol-owned net position",
            f"${po_position_notional_net:,.2f}",
            f"${abs(po_position_notional_net * 0.05):,.2f} at risk on 5% move",
        )

        p2.metric(
            "protocol-owned deposits",
            f"${po_deposits:,.2f}",
            f"${usdc_dep_rate * po_deposits:,.2f} interest per year",
        )
        p2.metric(
            "est borrow revenue",
            f"${total_borrow_fee_annualized:,.2f}",
            f"${total_borrow_fee_annualized / 365:,.2f} interest per day",
        )
            ...
            with tab:
                ...
                if all_users is not None:
                    df1 = (
                        ...
                        .sort_values("base_asset_amount", ascending=False)
                        .reset_index(drop=True)
                    )

                pct_long = market.amm.base_asset_amount_long / (
                ...
                st.text(
                    "User Perp Positions  (Base /|Quote) "
                    + str((market.number_of_users_with_base))
                    + " / "
                    + str((market.number_of_users))
                )
                ...
                if df1 is not None:
                    ...
                    df1["entry_price"] = -df1["quote_entry_amount"] / df1[
                        "base_asset_amount"
                    ].apply(lambda x: 1 if x == 0 else x)
                    df1["breakeven_price"] = -df1["quote_break_even_amount"] / df1[
                        "base_asset_amount"
                    ].apply(lambda x: 1 if x == 0 else x)
                    df1["cost_basis"] = -df1["quote_asset_amount"] / df1[
                        "base_asset_amount"
                    ].apply(lambda x: -1 if x == 0 else x)
                ...
                if perp_liq_prices_m is not None and len(perp_liq_prices_m) > 0:
                    ...
                    try:
                        oracle_price = (
                            chu.get_perp_oracle_data(perp_market)
                        ).price / PRICE_PRECISION
                    except:
                        oracle_price = (
                            perp_market.amm.historical_oracle_data.last_oracle_price
                    ...
                    max_price = int(
                        max(np.median(perp_liq_prices_m), oracle_price) * 1.3
                    )
                    max_price = st.number_input(
                        "max_price", value=max_price, key=1337 * (market_index + 1)
                    )
                    ...
                    n_bins = st.number_input(
                        "number of bins:",
                        value=min(100, len(perp_liq_prices_m)),
                        key=1 * (market_index + 1),
                    )
                    ...
                    fig = px.histogram(
                        perp_liq_prices_m,
                        nbins=n_bins,
                        labels={"x": "liq_price", "y": "count"},
                    )
                    fig.add_vline(
                        x=oracle_price, line_color="red", annotation_text="oracle price"
                    )
                    ...
                    for liq in liq_deltas:
                        if liq > 0:
                            _liq_deltas.append(min(max_pos_liq, liq))
                        if liq < 0:
                            _liq_deltas.append(max(max_neg_liq, liq))
        ...
        for market_index, tab in enumerate(tabs):
            ...
            with tab:
                ...
                if all_users is not None:
                    df1 = (
                        spots[
                            (spots.scaled_balance != 0)
                            & (spots.market_index == market_index)
                        ]
                        .sort_values("scaled_balance", ascending=False)
                        .reset_index(drop=True)
                    ...
                    sb = df1[
                        df1["balance_type"].map(str) == "SpotBalanceType.Deposit()"
                    ]
                    deposits = sb["scaled_balance"].values
                    total_deposits = 0
                    for depo in deposits:
                        token_amount = get_token_amount(
                            depo * 1e9, market, "SpotBalanceType.Deposit()"
                        )
                    ...
                    for borr in borrows:
                        token_amount = get_token_amount(
                            borr * 1e9, market, "SpotBalanceType.Borrow()"
                        )
                ...
                if (
                    spot_liq_prices_m is not None
                    and len(spot_liq_prices_m) > 0
                    and market_index != 0
                ):  # usdc (assumed to always be 1) doesnt really make sense
                    spot_market = chu.get_spot_market_account(market_index)
                    try:
                        oracle_price = (
                            chu.get_spot_oracle_data(spot_market).price
                            / PRICE_PRECISION
                        )
                    except:
                        oracle_price = (
                    ...
                    max_price = st.number_input(
                        "max_price",
                        value=max(oracle_price, np.median(spot_liq_prices_m)) * 1.3,
                        key=19 * (market_index + 1),
                    )
                    ...
                    n_bins = st.number_input(
                        "number of bins:",
                        value=min(100, len(spot_liq_prices_m)),
                        key=23 * (market_index + 1),
                    )
