...
def get_ir_curves(market, delt):
    deposits = (
        market.deposit_balance * market.cumulative_deposit_interest / 1e10 / (1e9)
    )
    ...
    bor_rate = (
        opt_borrow * (100 / opt_util) * utilization / 100
        if utilization <= opt_util
        else ((max_borrow - opt_borrow) * (100 / opt_util))
    ...
    dep_rate = (
        bor_rate * utilization * (1 - market.insurance_fund.total_factor / 1e6) / 100
    )
...
def calc_size_liab(wgt, imf, base):
    liability_wgt_n = wgt
    if imf != 0:
        liability_wgt_n = wgt * 0.8  # /(1/(imf/10000))
    dd = np.sqrt(np.abs(base))
...
def apy_to_apr(apy, compounds_per_year):
    ...
    est_apr = (
        compound_amount ** (1 / float(compounds_per_year)) - 1
    ) * compounds_per_year
    return est_apr
...
async def super_stake(clearing_house: DriftClient):
    ...
    msol_oracle_data = await get_oracle_price_data_and_slot(
        ch.program.provider.connection,
        msol_market.oracle,
        msol_market.oracle_source,
    )
    sol_oracle_data = await get_oracle_price_data_and_slot(
        ch.program.provider.connection, sol_market.oracle, sol_market.oracle_source
    )

    msol_oracle = msol_oracle_data.data
    sol_oracle = sol_oracle_data.data
    ...
    lev = c2.slider("leverage", 1.0, float(max_lev), step=0.05)
    ...
    divdown = msol_price * (
        (sol_maint_lwgt * ss_sol_bor) / (msol_maint_awgt * ss_msol_dep * msol_price)
    )
    ...
    with tabs[0]:
        ...
        aa = (ss_msol_dep * (stake_apr + msol_dep)) * msol_price - (
            ss_sol_bor * sol_bor
        )
        ...
        with st.expander("details:"):
            st.write(
                "`mSOL` initial asset weight:",
                msol_init_awgt,
                "`SOL` initial liability weight",
                sol_init_lwgt,
            )
            st.write(
                "`mSOL` maint asset weight:",
                msol_maint_awgt,
                "`SOL` maint liability weight",
                sol_maint_lwgt,
            )

            st.write(
                "`SOL`/`mSOL` oracle prices:",
                sol_oracle.price / 1e6,
                msol_oracle.price / 1e6,
            )
            st.write("`mSOL` fair value:", sol_oracle.price / 1e6 * msol_price)
            st.write(
                "initial weighted",
                msol_init_awgt * ss_msol_dep * msol_price - sol_init_lwgt * ss_sol_bor,
            )
            st.write(
                "maint weighted",
                msol_maint_awgt * ss_msol_dep * msol_price
                - sol_maint_lwgt * ss_sol_bor,
            )
            ...
            if ss_sol_bor > 0:
                divup = (
                    ...
                    * ss_msol_dep
                    * msol_price
                    / (sol_maint_lwgt * ss_sol_bor + 1e-9)
                    - 1
                ) * 100
                divdown = (
                    -(
                        (sol_maint_lwgt * ss_sol_bor)
                        / (msol_maint_awgt * ss_msol_dep * msol_price + 1e-9)
                        - 1
                    )
                    * 100
                )
    ...
    with tabs[1]:
        ...
        if byeq == "leverage":
            for x_lev in levs:
                ...
                msol_maint_awgt = calc_size_ast(msol_maint_awgt, msol_imf, ss_msol_dep)
                sol_init_lwgt = calc_size_liab(sol_init_lwgt, sol_imf, abs(ss_sol_bor))
                sol_maint_lwgt = calc_size_liab(
                    sol_maint_lwgt, sol_imf, abs(ss_sol_bor)
                )

                divup = (
                    abs(
                        (sol_maint_lwgt * ss_sol_bor) / (msol_maint_awgt * ss_msol_dep)
                        - msol_price
                    )
                    * 100
                )
                ...
                dd2.append(divup)
            ans = pd.DataFrame(
                [dd1, dd2], index=["mSOL depegs down", "SOL depegs up"], columns=levs
            ).T
        elif byeq == "size":
            ...
            for x_size in sizes:
                ...
                dd2.append(divup)
            ans = pd.DataFrame(
                [dd1, dd2], index=["mSOL depegs down", "SOL depegs up"], columns=sizes
            ).T

        st.write(
            '"depegs" means while one asset (mSOL or SOL) stays in same in value, the other one diverges'
        )
        st.write(
            "note that mSOL depegging higher and SOL depegging lower do not risk this position"
        )
        ...
        z_data["SOL_liq_price"] = (
            sol_oracle.price + ans["SOL depegs up"] * sol_oracle.price / 100
        ) / 1e6
        z_data["mSOL_liq_price"] = (
            msol_oracle.price - ans["mSOL depegs down"] * msol_oracle.price / 100
        ) / 1e6
        ...
        fig3.add_hline(
            y=curpx,
            line_color="green",
            annotation_text="current mSOL/SOL price = " + str(curpx)[:8],
        )
