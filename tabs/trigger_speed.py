...
def trigger_speed_analysis():
    ...
    if selected_market and start_date and end_date:
        ...
        for cohort in all_cohorts:
            data_df = fetch_trigger_speed_data_dynamodb(
        ...
        (trigger_limit_liquidity_source_data, trigger_market_liquidity_source_data) = (
            render_liquidity_source_analysis(
                start_ts, end_ts, selected_market, order_type
            )
        )
        ...
        for i, (cohort, data_df) in enumerate(cohort_items):
            if not data_df.empty:
                ...
                col4.metric(
                    ...
                    f"{total_triggered:,.0f}"
                    if not pd.isna(total_triggered)
                    else "N/A",
                )
...
def render_trigger_latency(toggle_use_p99, processed_cohort_data):
    ...
    selected_fillvstrigger_percentiles = fillvstrigger_trend_line_options[
        selected_fillvstrigger_trend_option
    ]
    ...
    for cohort, data_df in processed_cohort_data.items():
        if len(data_df) > 1:  # Only add trend lines if we have multiple data points
            ...
            for percentile_col in selected_fillvstrigger_percentiles:
                if (
                    percentile_col in fillvstrigger_percentiles_config
                    and percentile_col in data_df.columns
                    and not data_df[percentile_col].isnull().all()
                ):
                    percentile_name, line_style = fillvstrigger_percentiles_config[
                        percentile_col
                    ]
                    ...
                    y_values = sorted_df[percentile_col]
                    if (
                        fillvstrigger_smoothing_window > 1
                        and len(sorted_df) >= fillvstrigger_smoothing_window
                    ):
                        # Apply rolling average smoothing
                        y_values = (
                            sorted_df[percentile_col]
    ...
    if selected_fillvstrigger_trend_option != "None":
        if selected_fillvstrigger_trend_option == "All":
            ...
            fillvstrigger_title_suffix = (
                f" with {selected_fillvstrigger_trend_option} Trend Lines"
            )
    ...
    if selected_fillvstrigger_trend_option != "None" and len(processed_cohort_data) > 0:
        ...
        if fillvstrigger_slope_data:
            ...
            for i, (key, slope) in enumerate(fillvstrigger_slope_data.items()):
                ...
                with cols[col_idx]:
                    # Determine color based on slope direction
                    # For fill vs trigger price difference, an increasing positive value is generally bad (more slippage),
                    # while a decreasing value is good. We want it to be small and stable.
                    abs_slope = abs(slope)

                    if abs_slope < 0.01:  # A very small change is considered stable
                        trend_text = "Stable"
                        delta_color = "off"  # Use neutral color for stable
                    elif slope > 0:
...
def render_liquidity_source_analysis(start_ts, end_ts, selected_market, order_type):
    ...
    if order_type in ["all", "triggerMarket"]:
        trigger_market_liquidity_source_data = fetch_liquidity_source_data_dynamodb(
            ...
            selected_market,
            cohort="all",
            taker_order_type="triggerMarket",
            bit_flag="all",
        )
    ...
    def process_liquidity_data(df, title_prefix):
        if df.empty:
            st.warning(
                f"No {title_prefix.lower()} liquidity source data available for the selected period."
            )
            return
        ...
        available_sources = [
            ...
            if col in df_processed.columns and not df_processed[col].isnull().all()
        ]

        if not available_sources:
            st.warning(
                f"No valid liquidity source data found for {title_prefix.lower()} orders."
            )
            return
        ...
        for col in available_sources:
            df_processed[f"{col}_pct"] = (
                df_processed[col] / df_processed["total_volume"] * 100
            ).fillna(0)
        ...
        with st.expander(
            f"📊 {title_prefix} Liquidity Source Summary Statistics", expanded=False
        ):
    ...
    if (
        order_type in ["all", "triggerMarket"]
        and not trigger_market_liquidity_source_data.empty
    ):
        process_liquidity_data(trigger_market_liquidity_source_data, "Trigger Market")

    if (
        order_type in ["all", "triggerLimit"]
        and not trigger_limit_liquidity_source_data.empty
    ):
