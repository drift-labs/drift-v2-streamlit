    ...
    if len(user_authorities):
        ...
        for user_authority in user_authorities:
            ...
            for sub_id in range(user_stat.number_of_sub_accounts_created):
                print("sub_id:", sub_id)
                user_account_pk = get_user_account_public_key(
                    ch.program.program_id, user_authority_pk, sub_id
                )
                chu_sub = DriftUser(
                    ch,
                ...
                if sub_id == 0:
                    ...
                    df["baseAssetAmountSignedFilled"] = (
                        ...
                        * df["taker"].apply(
                            lambda x: 2 * (str(x) == user_account_pk) - 1
                        )
                    )
                    ...
                    tt = (
                        df.groupby(["takerOrderId", "takerOrderDirection"])
                        .count()
                        .iloc[:, 0]
                        .reset_index()
                    )
                    ...
                    trades = df.groupby(["marketType", "marketIndex"]).sum()[
                        ["quoteAssetAmountFilled"]
                    ]
                    ...
                    with tabs[1]:
                        st.plotly_chart(
                            ot.plot(title="base asset amount over month (WIP)")
                        )
                        st.dataframe(tt1)
                        st.dataframe(trades)
                ...
                summary["total_collateral"] = (
                    await chu_sub.get_total_collateral()
                ) / 1e6
                summary["init total_collateral"] = (
                    await chu_sub.get_total_collateral(MarginCategory.INITIAL)
                ) / 1e6
                summary["maint total_collateral"] = (
                    await chu_sub.get_total_collateral(MarginCategory.MAINTENANCE)
                ) / 1e6
                summary["initial_margin_req"] = (
                    await chu_sub.get_margin_requirement(
                        MarginCategory.INITIAL, None, True, False
                    )
                ) / 1e6
                summary["maintenance_margin_req"] = (
                    await chu_sub.get_margin_requirement(
                ...
                for spot_market_index in range(state.number_of_spot_markets):
                    ...
                    if spot_pos is not None:
                        ...
                        tokens = get_token_amount(
                            spot_pos.scaled_balance, spot, str(spot_pos.balance_type)
                        )
                        ...
                        dd = {
                            ...
                            "net usd value": await chu_sub.get_spot_market_asset_value(
                                None, False, spot_market_index
                            )
                            / 1e6,
                            "asset weight": calculate_asset_weight(tokens, spot, None)
                            / 1e4,
                            "initial asset weight": calculate_asset_weight(
                                tokens, spot, MarginCategory.INITIAL
                            )
                            / 1e4,
                            "maint asset weight": spot.maintenance_asset_weight / 1e4,
                            "your maint asset weight": calculate_asset_weight(
                                tokens, spot, MarginCategory.MAINTENANCE
                            )
                            / 1e4,
                            "weighted usd value": await chu_sub.get_spot_market_asset_value(
                                MarginCategory.INITIAL, False, spot_market_index
                            )
                            / 1e6,
                            "maint weighted usd value": await chu_sub.get_spot_market_asset_value(
                                MarginCategory.MAINTENANCE, False, spot_market_index
                            )
                            / 1e6,
                            "liq price": await chu_sub.get_spot_liq_price(
                                spot_market_index
                            ),
                        }
                        balances.append(dd)
                ...
                for perp_market_index in range(state.number_of_markets):
                    ...
                    if pos is not None:
                        ...
                        dd2 = {
                            ...
                            "liq price": await chu_sub.get_perp_liq_price(
                                perp_market_index
                            ),
                        }
                        z = {**dd, **dd2}
        ...
        with tabs[0]:
            ...
            if len(all_summarys) > 0:
                ...
                st.metric(
                    "total account value:",
                    "$" + str(int(sub_dfs["total_collateral"].sum() * 100) / 100),
                )
