...
async def get_usermap_df(
    ...
    def do_dict(x: DriftUser, margin_category: MarginCategory, oracle_cache=None):
        ...
        levs0 = {
            ...
            "total_collateral": x.get_total_collateral(
                margin_category or MarginCategory.INITIAL
            )
            / QUOTE_PRECISION,
            "margin_req": x.get_margin_requirement(
                margin_category or MarginCategory.INITIAL
            )
            / QUOTE_PRECISION,
            ...
            "cumulative_perp_funding": user_account.cumulative_perp_funding
            / QUOTE_PRECISION,
            ...
            "unsettled_pnl_perp_x": x.get_unrealized_pnl(True, market_index=24)
            / QUOTE_PRECISION,
        }
        levs0["net_usd_value"] = (
            levs0["spot_asset"] + levs0["upnl"] - levs0["spot_liability"]
        )
        return levs0
...
@st.cache_data
def cached_get_usermap_df(
    _drift_client, user_map_settings, mode, oracle_distort, only_one_index, cov_matrix
):
...
async def get_protocol_summary(drift_client: DriftClient):
    ...
    for market_index in range(state.number_of_spot_markets):
        ...
        sv_amount = await load_token_balance(
            drift_client.program.provider.connection, svault_pk
        )
        sv_amount /= 10**market.decimals
        px1 = (
            drift_client.get_oracle_price_data_for_spot_market(market_index)
        ).price / 1e6
        sdep = (market.deposit_balance * market.cumulative_deposit_interest / 1e10) / (
            1e9
        )
        sbor = (market.borrow_balance * market.cumulative_borrow_interest / 1e10) / (
            1e9
        )
    ...
    for market_index in range(state.number_of_markets):
        ...
        px1 = (
            drift_client.get_oracle_price_data_for_perp_market(market_index)
        ).price / 1e6
        fee_pool = (
...
def usermap_page(drift_client: DriftClient, env):
    ...
    user_map_settings = s11.radio(
        "user map settings:", ["all", "active", "idle", "whales"], index=1
    )
    ...
    if mode == "margin":
        ...
        oracle_distort = s4.slider(
            ...
            0.1,
            step=0.05,
            help="3 intervals of the this step will be used for price scenario analysis",
        )

        all_symbols = sorted(
            list(
                set(
                    [x.base_asset_symbol for x in mainnet_perp_market_configs]
                    + [x.symbol.replace("w", "") for x in mainnet_spot_market_configs]
                )
            )
        )
        only_one_index = s4.selectbox(
            "only single oracle:",
            (
                [None] + all_symbols
    ...
    levs, user_keys = cached_get_usermap_df(
        drift_client,
        user_map_settings,
        mode,
        oracle_distort,
    ...
    (
        ...
        spot_acct_bor,
        num_subs,
    ) = cached_get_protocol_summary(drift_client)

    tabs = st.tabs(
        [
            "summary",
            "price scenario bankruptcies",
            "leverage",
            "spot0",
            "scatter",
            "raw tables",
    ...
    with tabs[0]:
        ...
        col1.metric(
            f"{len(df)}/{num_subs} of users tvl",
            f"${df['net_usd_value'].sum():,.2f}",
            f"${user_usdc_total:,.2f} USDC total (excess={usdc_excess})",
        )
        col11.metric(
            "state tvl",
            f"${spot_acct_dep + spot_acct_bor:,.2f}",
            f"{-spot_acct_bor / spot_acct_dep * 100:,.2f}% utilization",
        )
        col2.metric(
            "vault tvl",
            f"${aa:,.2f}",
            f"{aa - (spot_acct_dep + spot_acct_bor):,.2f} excess vs state",
        )
        col3.metric(
            "vamm tvl",
            f"${vamm + vamm_upnl:,.2f}",
            f"${vamm:,.2f} tvl + {vamm_upnl:,.2f} upnl",
        )
        vamm2 = spot_acct_dep + spot_acct_bor - df["net_usd_value"].sum()
        col4.metric(
            "vamm2 tvl", f"${vamm2:,.2f}", f"{vamm2 - (vamm + vamm_upnl):,.2f} excess"
        )
        ...
        s3.metric(
            "bankruptcies:",
            f"${bankrupts['net_usd_value'].sum():,.2f}",
            f"{len(bankrupts)} unique accounts",
        )
        s3.dataframe(bankrupts)
        st.write(df["tokens"])
        recon_tokens = [
            df["tokens"].apply(lambda x: x[i]).sum()
            / (10 ** drift_client.get_spot_market_account(i).decimals)
            for i in range(len(spot_vaults.keys()))
        ]
        net_v2 = [
            df["net_v"].apply(lambda x: x[i]).sum()
            for i in range(len(spot_vaults.keys()))
        ]
        aas2 = [spot_vaults["spot" + str(i)] for i in range(len(spot_vaults.keys()))]
        states2 = [spot_states["spot" + str(i)] for i in range(len(spot_vaults.keys()))]
        recon_df = pd.DataFrame(
            [recon_tokens, net_v2, aas2, states2],
            index=["user_tokens", "user_value", "vault_tokens", "state_tokens"],
        ).T
        ...
        value_of_users = (
            df[["net_usd_value"]].sort_values(by="net_usd_value").reset_index(drop=True)
        )
        value_of_users["cumulative_usd_value"] = value_of_users[
            "net_usd_value"
        ].cumsum()
    ...
    def get_rattt(row):
        ...
        df1 = pd.Series(
            [
                ...
                * (row["perp_liability"] + row["spot_liability"])
                ...
                for key, val in row["net_v"].items()
            ]
        )
        ...
        df2 = pd.Series(
            [
                val / (row["spot_asset"]) * (row["perp_liability"]) if val > 0 else 0
                for key, val in row["net_v"].items()
            ]
        )
        ...
        df3 = pd.Series(
            [
                val / (row["spot_asset"]) * (row["spot_liability"]) if val > 0 else 0
                for key, val in row["net_v"].items()
            ]
        )
        ...
        df4 = pd.Series(
            [
                val / (row["spot_asset"]) * (row["net_p"][perp_market_inspect])
                if val > 0 and row["net_p"][0] > 0
                else 0
                for key, val in row["net_v"].items()
            ]
        )
        df4.index = [
            "spot_" + str(x) + "_perp_" + str(perp_market_inspect) + "_long"
            for x in df4.index
        ]
        ...
        df5 = pd.Series(
            [
                val / (row["spot_asset"]) * (row["net_p"][perp_market_inspect])
                if val > 0 and row["net_p"][perp_market_inspect] < 0
                else 0
                for key, val in row["net_v"].items()
            ]
        )
        df5.index = [
            "spot_" + str(x) + "_perp_" + str(perp_market_inspect) + "_short"
            for x in df5.index
        ]
    ...
    res = pd.DataFrame(
        {
            ("spot" + str(i)): (
                df["spot_" + str(i) + "_all"].sum(),
                df["spot_" + str(i) + "_all_spot"].sum(),
                df["spot_" + str(i) + "_all_perp"].sum(),
                df[
                    "spot_" + str(i) + "_perp_" + str(perp_market_inspect) + "_long"
                ].sum(),
                df[
                    "spot_" + str(i) + "_perp_" + str(perp_market_inspect) + "_short"
                ].sum(),
            )
    ...
    with tabs[1]:
        ...
        xdf = [
            ...
            [
                -(
                    df[
                        (df["spot_asset"] < df["spot_liability"])
                        & (df["net_usd_value"] < 0)
                    ]["net_usd_value"].sum()
                )
                for df in dfs
        ...
        toplt_fig["perp bankrupt"] = (
            toplt_fig["bankruptcy"] - toplt_fig["spot bankrupt"]
        )
    ...
    with tabs[2]:
        ...
        lev_one_market = df["net_p"].apply(
            lambda x: abs(x[perp_market_inspect])
            if x[perp_market_inspect] != 0
            else np.nan
        ) / (df["spot_asset"] - df["spot_liability"] + df["upnl"])
    ...
    with tabs[4]:
        ...
        fig = px.scatter(
            ...
            y="spot_asset",
            size=col,
            hover_data=["index", "leverage", col, "spot_asset", "spot_liability"],
        )

        # Customize the layout if needed
        fig.update_layout(
            title="Bubble Plot of Size and SpotIi",
            xaxis_title="Size",
            yaxis_title="SpotIi",
        )
        st.plotly_chart(fig)
    ...
    with tabs[6]:
        ...
        perp_market_inspect_dlp = st.selectbox(
            "perp market:", list(range(NUMBER_OF_PERP)), index=10, key="tab-5"
        )
        st.write("dlp with positions in perp market=", perp_market_inspect_dlp)

        dlp_one_market = dlp_df[
            dlp_df.net_lp.apply(lambda x: x[perp_market_inspect_dlp] != 0)
        ]
        st.write(dlp_one_market)

        st.write(
            "total lp users:",
            dlp_df.net_lp.apply(lambda x: x[perp_market_inspect_dlp] != 0).sum(),
        )
        st.write(
            "total user lp shares:",
            dlp_df.net_lp.apply(lambda x: x[perp_market_inspect_dlp]).sum(),
        )

        dlp_market_account = drift_client.get_perp_market_account(
            perp_market_inspect_dlp
        )
        st.write(
            "total lp shares (market)", dlp_market_account.amm.user_lp_shares / 1e9
        )
