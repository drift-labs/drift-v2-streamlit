...
def load_deposit_history(userkey, source="api"):
    if source == "api":
        url = (
            "https://mainnet-beta.api.drift.trade/deposits/userAccounts/?userPublicKeys="
            + userkey
            + "&pageIndex=0&pageSize=1000&sinceId=&sinceTs="
        )
...
def make_buy_sell_chart(usr_to_show):
    ...
    usr_to_show.index = pd.to_datetime(
        (usr_to_show.first_ts * 1e9).astype(int), utc=True
    )
    ...
    for mt in usr_to_show.marketType.unique():
        for mi in usr_to_show.marketIndex.unique():
            uu = usr_to_show[
                (usr_to_show.marketIndex == mi) & (usr_to_show.marketType == mt)
            ]
            signals = uu.direction.apply(
            ...
            signal_trace = go.Scatter(
                ...
                marker=dict(
                    symbol=[
                        "triangle-up" if s == 1 else "triangle-down" for s in signals
                    ],
...
def build_liquidity_status_df(df, user_account_pk):
    def stitch_liq_status():
        ...
        if "takerOrderDirection" in user_trades_full.columns:
            user_trades_full.loc[
                user_trades_full.liquidityStatus == "taker", "direction"
            ] = user_trades_full.loc[
                user_trades_full.liquidityStatus == "taker", "takerOrderDirection"
            ]
            user_trades_full.loc[
                user_trades_full.liquidityStatus == "taker", "size"
            ] = user_trades_full.loc[
                user_trades_full.liquidityStatus == "taker",
                "takerOrderCumulativeBaseAssetAmountFilled",
            ]
            user_trades_full.loc[
                user_trades_full.liquidityStatus == "taker", "notional"
            ] = user_trades_full.loc[
                user_trades_full.liquidityStatus == "taker",
                "takerOrderCumulativeQuoteAssetAmountFilled",
            ]
            user_trades_full.loc[user_trades_full.liquidityStatus == "taker", "fee"] = (
                user_trades_full.loc[
                    user_trades_full.liquidityStatus == "taker", "cumulativeTakerFee"
                ]
            )
        if "makerOrderDirection" in user_trades_full.columns:
            user_trades_full.loc[
                user_trades_full.liquidityStatus == "maker", "direction"
            ] = user_trades_full.loc[
                user_trades_full.liquidityStatus == "maker", "makerOrderDirection"
            ]
            user_trades_full.loc[
                user_trades_full.liquidityStatus == "maker", "size"
            ] = user_trades_full.loc[
                user_trades_full.liquidityStatus == "maker",
                "makerOrderCumulativeBaseAssetAmountFilled",
            ]
            user_trades_full.loc[
                user_trades_full.liquidityStatus == "maker", "notional"
            ] = user_trades_full.loc[
                user_trades_full.liquidityStatus == "maker",
                "makerOrderCumulativeQuoteAssetAmountFilled",
            ]
            user_trades_full.loc[user_trades_full.liquidityStatus == "maker", "fee"] = (
                user_trades_full.loc[
                    user_trades_full.liquidityStatus == "maker", "cumulativeMakerFee"
                ]
            )
    ...
    def make_liq_status_df(status, is_liq=None):
        ...
        user_takersd["orderFillPrice"] = (
            user_takersd[status + "OrderCumulativeQuoteAssetAmountFilled"]
            / user_takersd[status + "OrderCumulativeBaseAssetAmountFilled"]
        )
...
async def show_user_perf(clearing_house: DriftClient):
    ...
    if len(user_authorities):
        ...
        dates = []
        sub = subaccount0.selectbox(
            "subaccount", [x.split("_")[-1] for x in uak_df.index]
        )
        ...
        for user_authority in user_authorities:
            ...
            for sub_id in range(user_stat.number_of_sub_accounts_created):
                ...
                if sub_id == int(sub):
                    ...
                    st.write('"' + nom + '"')
                    tabs1 = st.tabs(
                        ["trades", "deposits", "all time stats", "snapshot"]
                    )
                    ...
                    with tabs1[1]:
                        ...
                        df["amount2"] = df["amount"] * df["direction"].apply(
                            lambda x: -1 if x != "deposit" else 1
                        )
                        df2 = df.pivot_table(
                            index="ts", columns="marketIndex", values="amount2"
                        )
                        ...
                        df2.index = pd.to_datetime(
                            (df2.index.astype(int) * 1e9).astype(int), utc=True
                        )
                        ...
                        for i, c in enumerate(ccs):
                            ...
                            current_price = (
                                chu.get_oracle_data_for_spot_market(mi)
                            ).price / PRICE_PRECISION
                            atnd_token_current_value += (
                                net_dep_amounts.iloc[i] * current_price
                            )
                            c.metric(
                                "net deposit tokens (market_index=" + str(mi) + ")",
                                net_dep_amounts.iloc[i],
                            )
                    ...
                    with tabs1[2]:
                        atnd = (
                            user_acct.total_deposits - user_acct.total_withdraws
                        ) / 1e6
                        ...
                        for idx in range(len(user_acct.spot_positions)):
                            ...
                            if spot_pos.cumulative_deposits != 0:
                                spot_market_act = await get_spot_market_account(
                                    ch.program, spot_pos.market_index
                                )
                                scale = 10**spot_market_act.decimals
                                spot_market_name = bytes(spot_market_act.name).decode(
                                    "utf-8"
                                )
                                if (
                                    ...
                                    tokens = (
                                        ...
                                        * spot_market_act.cumulative_deposit_interest
                                        / 1e10
                                        / (1e9)
                                    )
                                else:
                                    tokens = (
                                        -spot_pos.scaled_balance
                                        * spot_market_act.cumulative_borrow_interest
                                        / 1e10
                                        / (1e9)
                                    )

                                gains_since_inception[spot_pos.market_index] = (
                                    tokens - (spot_pos.cumulative_deposits / scale)
                                )
                                s3.metric(
                                    spot_market_name + " deposits:",
                                    f"{tokens:,.3f}",
                        ...
                        excess_usdc_gains_since_inception = (
                            gains_since_inception.get(0, 0) - atsp
                        )

                        st.write(
                            "excess usdc gains:", excess_usdc_gains_since_inception
                        )
                    ...
                    with tabs1[0]:
                        ...
                        if mt == "perp":
                            mi = micol.selectbox(
                                "market index:", range(0, state.number_of_markets)
                            )
                        elif mt == "spot":
                            mi = micol.selectbox(
                                "market index:", range(1, state.number_of_spot_markets)
                            )
                            sources = ["trades"]
                        ...
                        if source == "trades" or source == "all":
                            df, urls = load_user_trades(
                                dates, str(user_account_pk), True
                            )
                            if len(df) == 0:
                                continue
                            ...
                            df = filter_dups(df)
                            df["baseAssetAmountSignedFilled"] = (
                                df["baseAssetAmountFilled"]
                                * df["takerOrderDirection"].apply(
                                    lambda x: 2 * (x == "long") - 1
                                )
                                * df["taker"].apply(
                                    lambda x: 2 * (str(x) == str(user_account_pk)) - 1
                                )
                            )
                            df["quoteAssetAmountSignedFilled"] = (
                                df["quoteAssetAmountFilled"]
                                * df["takerOrderDirection"].apply(
                                    lambda x: 2 * (x == "long") - 1
                                )
                                * df["taker"].apply(
                            ...
                            user_trades_full = build_liquidity_status_df(
                                df, user_account_pk
                            )
                            if len(user_trades_full):
                                usr_to_show = user_trades_full[
                                    [
                                        "marketType",
                                        "marketIndex",
                                        "direction",
                                ...
                                st.dataframe(
                                    usr_to_show.reset_index(drop=True).style.apply(
                                        highlight_survived, axis=1
                                    )
                                )
                                ...
                                ot = (
                                    df.pivot_table(
                                        index="ts",
                                        columns=["marketType", "marketIndex"],
                                        values="baseAssetAmountSignedFilled",
                                        aggfunc="sum",
                                    )
                                    .cumsum()
                                    .ffill()
                                ...
                                ot.index = pd.to_datetime(
                                    (ot.index * 1e9).astype(int), utc=True
                                )
                                ...
                                cost_ser = (
                                    df.pivot_table(
                                        index="ts",
                                        columns=["marketType", "marketIndex"],
                                        values="quoteAssetAmountSignedFilled",
                                        aggfunc="sum",
                                    )
                                    .cumsum()
                                    .ffill()
                                ...
                                cost_ser.index = pd.to_datetime(
                                    (cost_ser.index * 1e9).astype(int), utc=True
                                )

                                px_ser = df.pivot_table(
                                    index="ts",
                                    columns=["marketType", "marketIndex"],
                                    values="oraclePrice",
                                    aggfunc="last",
                                ...
                                px_ser.index = pd.to_datetime(
                                    (px_ser.index * 1e9).astype(int), utc=True
                                )
                            ...
                            sdf_tot0 = sdf_tot.swaplevel(
                                axis=1
                            )  # put market index on top
                            mis = None
                            if mi is not None:
                                # sdf_tot = sdf_tot[mi]
                            ...
                            for mi in mis:
                                sdf_tot = sdf_tot0[mi]
                                sdf_tot.index = pd.to_datetime(
                                    (sdf_tot.index * 1e9).astype(int), utc=True
                                )
                                sdf_tot = sdf_tot.loc[dates[0] : dates[1]]

                                # st.dataframe(sdf_tot)
                                sdf_tot["costBasisAfter"] = (
                                    -(sdf_tot["quoteEntryAmount"])
                                    / sdf_tot["baseAssetAmount"]
                                ).fillna(0)
                                sdf_tot["cumulativePnl"] = sdf_tot["pnl"].cumsum()
                                sdf_tot["notional"] = (
                                    sdf_tot["baseAssetAmount"] * sdf_tot["settlePrice"]
                                )

                                sdf_tot["entryPnl"] = (
                                    sdf_tot["notional"] + sdf_tot["quoteEntryAmount"]
                                )
