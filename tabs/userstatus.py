...
from helpers import all_user_stats
...
async def userstatus_page(ch: DriftClient):
    ...
    oracle_distort = s2.slider(
        "base oracle distortion:",
        0.01,
        2.0,
        1.0,
        0.1,
        help="alter non-stable token oracles by this factor multiple (default=1, no alteration)",
    )
    ...
    async def load_users(filt):
        if filt == "All":
            ...
            all_users = await _ch_user_act.all(
                filters=[MemcmpOpts(offset=4350, bytes="2")]
            )
            ...
            all_users = await _ch_user_act.all(
                filters=[MemcmpOpts(offset=4354, bytes="2")]
            )
        return all_users
    ...
    def is_proper_super_staker(x):
        ...
        if (
            no_perp.base_asset_amount != 0
            or no_perp.quote_asset_amount != 0
            or no_perp.open_orders != 0
            or no_perp.lp_shares != 0
        ):
            return False
    ...
    with tabs[0]:
        ...
        col1.metric(
            ...
            filt.lower() + " users = " + str(len(all_users)),
        )
        ...
        col2.metric(
            "user tvl", f"${user_tvl:,.2f}", f"${stats_df.upnl.sum():,.2f} upnl"
        )
        ...
        for market_index in range(num_markets):
            ...
            sv_amount = await load_token_balance(
                ch.program.provider.connection, svault_pk
            )
            sv_amount /= 10**market.decimals
            px1 = (chu.get_oracle_data_for_spot_market(market_index)).price / 1e6
            sv_amount *= px1
            spot_acct_dep += (
                (market.deposit_balance * market.cumulative_deposit_interest / 1e10)
                / (1e9)
                * px1
            )
            spot_acct_bor -= (
                (market.borrow_balance * market.cumulative_borrow_interest / 1e10)
                / (1e9)
                * px1
            )
        ...
        stats_df.loc[
            ((stats_df["upnl"] < 0) & (stats_df["spot_value"] < -stats_df["upnl"])),
            "perp bankrupt",
        ] = True
        ...
        st.write(
            ...
            "are subaccount id = 0",
            "($",
            stats_df.loc[new_accounts["public_key"].astype(str).values][
                "spot_value"
            ].sum(),
            ")",
        )
        st.write(
            len(stats_df[stats_df["spot_value"] > 0]),
            "have a balance",
            "($",
            stats_df[stats_df["spot_value"] > 0].sum(),
            ")",
        )
        ...
        df111 = pd.concat(
            {
                "leverage": (
                    (stats_df["total_liability"] + stats_df["spot_value"])
                    / stats_df["spot_value"]
                ),
        ...
        dod = df111.sum()
        dod = (
            stats_df["total_liability"].sum() + stats_df["spot_value"].sum()
        ) / stats_df["spot_value"].sum()
        fig111 = px.scatter(
            df111,
            x="leverage",
            y="notional_size",
    ...
    with tabs[2]:
        ...
        omin, omax = scol.slider(
            "Select an oracle multiplier range", 0.0, 10.0, (0.0, 2.0), step=0.05
        )

        only_one_index = scc.selectbox(
            "only single oracle",
            (
                [None]
                + list(
                    chu.drift_client.account_subscriber.cache[
                        "oracle_price_data"
                    ].keys()
                )
            ),
        ...
        oracle_distorts = [
            float(x) / 10 for x in (list(np.linspace(omin, omax * 10, 100)))
        ]
        all_stats = {}
        ...
        for oracle_distort_x in oracle_distorts:
            stats_df, chunew = await all_user_stats(
                all_users,
                ch,
        ...
        statdf = pd.DataFrame(
            all_stats, index=["bankruptcy", "spot bankruptcy", "perp bankruptcy"]
        ).T.abs()
        nom = ""
        if only_one_index is not None:
            perp_market_i_data = [
                x
                for x in chu.drift_client.account_subscriber.cache["perp_markets"]
                if str(x.data.amm.oracle) == only_one_index
            ][0].data
            nom = bytes(perp_market_i_data.name).decode("utf-8").strip(" ") + " "
            perp_market_account_if = perp_market_i_data.insurance_claim
            max_if_perp_payment = (
                perp_market_account_if.quote_max_insurance
                - perp_market_account_if.quote_settled_insurance
            ) / 1e6
            statdf["max insurance claim"] = (
                statdf["perp bankruptcy"].clip(0, max_if_perp_payment)
                + statdf["spot bankruptcy"]
            )
        fig = statdf.plot()
        fig.update_layout(
