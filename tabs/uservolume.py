...
async def fetch_lp_data(date, market_name):
    market_index = [
        x.market_index for x in mainnet_perp_market_configs if market_name == x.symbol
    ]
...
async def load_lp_info_async(dates, market_names, with_urls=False):
    ...
    tasks = [
        fetch_lp_data(date, market_name)
...
async def load_volumes_async(dates, market_names, with_urls=False):
    ...
    tasks = [
        fetch_volume_data(date, market_name)
        for market_name in market_names
        for date in dates
    ]
...
async def show_user_volume(clearing_house: DriftClient):
    ...
    market_name = mol1.selectbox(
        "market", ["All", "All PERP", "All SPOT"] + ALL_MARKET_NAMES, index=3
    )
    ...
    tabs = st.tabs(
        ["overview", "user breakdown", "fill quality", "score", "snapshot inspect"]
    )
    ...
    if selected == "Volume":
        ...
        dfs.loc[dfs["taker"] == "undefined", "taker"] = (
            dfs.loc[dfs["taker"] == "undefined", "actionExplanation"]
            .map(mapping_dict)
            .fillna("external")
        )
        dfs.loc[dfs["maker"] == "undefined", "maker"] = (
            dfs.loc[dfs["maker"] == "undefined", "actionExplanation"]
            .map(mapping_dict)
            .fillna("external")
        )
        ...
        maker_jit_volume = (
            dfs[dfs["actionExplanation"] == "orderFilledWithMatchJit"]
            .groupby("maker")["quoteAssetAmountFilled"]
            .sum()
            .fillna(0)
        )
        ...
        volume_df["total volume"] = volume_df[["maker volume", "taker volume"]].sum(
            axis=1
        )
        volume_df = volume_df[
            ["total volume", "taker volume", "maker volume", "maker jit volume"]
        ]
    ...
    with tabs[0]:
        ...
        s1.metric(
            "total volume:",
            f"${dfs['quoteAssetAmountFilled'].sum():,.2f}",
            f"{len(dfs):,} trades",
        )
        s2.metric(
            "total fees:",
            f"${dfs[['takerFee']].sum().sum():,.2f} taker",
            f"{len(dfs['taker'].unique())} unique takers",
        )
        s3.metric(
            "total rebates:",
            f"${dfs[['makerFee']].sum().sum():,.2f} maker",
        ...
        fill_types = (
            dfs.groupby("actionExplanation")["quoteAssetAmountFilled"]
            .sum()
        ...
        f2.write(
            dfs[dfs.fillerReward > 0]
            .groupby("filler")
            .agg({"fillerReward": "sum", "ts": "count"})
    ...
    with tabs[1]:
        if selected == "Volume":
            ...
            s1.metric(
                "total volume:",
                f"${volume_df['taker volume'].sum():,.2f}",
            ...
            volumes_plotted = (
                volume_df[["taker volume"]]
                .dropna()
                .reset_index(drop=True)
                .sort_values(by="taker volume")
            )
            volumes_plotted["cumulative"] = (
                volume_df["taker volume"].reset_index(drop=True).fillna(0).cumsum()
            )
            ...
            st.write(
                volume_df[
                    (volume_df["total volume"] >= a_min)
                    & (volume_df["total volume"] <= a)
                ]["taker volume"].sum(),
        ...
        if selected == "Referrals":
            ...
            if "referrerReward" in dfs.columns:
                reward_volume = pd.DataFrame(
                    dfs[
                        dfs.referrerReward.replace("undefined", 0)
                        .fillna(0)
                        .astype(float)
                        > 0
                    ]
                ...
                if opt == "by referrer":
                    ...
                    reward_volume.index = reward_volume["taker"].apply(
                        lambda x: str(
                            user_stats_df[user_stats_df["subAccounts"].str.contains(x)][
                                "referrer"
                            ]
                            .dropna()
                            .values[0]
                    ...
                    reward_volume = (
                        reward_volume.reset_index().groupby("referrer").sum()
                    )
                ...
                if opt == "by referrer":
                    with st.expander("referrer map"):
                        st.dataframe(
                            user_stats_df[["authority", "subAccounts", "referrer"]]
                        )
            else:
        ...
        if selected == "Volume":
            ...
            if auth_maker_df.shape[0] > 0 or auth_taker_df.shape[0] > 0:
                ...
                if not auth_maker_df.empty:
                    ...
                    auth_maker_df["execPrice"] = auth_maker_df.apply(
                        calc_maker_exec_price, axis=1
                    )
                    auth_maker_df["ts"] = pd.to_datetime(
                        auth_maker_df["ts"], unit="s", utc=True
                    )
                        ...
                        pd.DataFrame(
                            {
                                ...
                                "makerOrderDirection": auth_maker_df[
                                    "makerOrderDirection"
                                ],
                                "taker": auth_maker_df["taker"],
                                "takerOrderId": auth_maker_df["takerOrderId"],
                                "takerOrderDirection": auth_maker_df[
                                    "takerOrderDirection"
                                ],
                                "baseAssetAmountFilled": auth_maker_df[
                                    "baseAssetAmountFilled"
                                ],
                                "quoteAssetAmountFilled": auth_maker_df[
                    ...
                    counterparty_df = calculate_agg_counterparty_volume(
                        auth_maker_df, "maker"
                    )

                    fig = px.pie(
                        counterparty_df,
                        values="baseAssetAmountFilled",
                        title="Counterparty Base Volume",
                        names="taker",
                    )
                    ...
                    fig = px.pie(
                        counterparty_df,
                        values="quoteAssetAmountFilled",
                        title="Counterparty Quote Volume",
                        names="taker",
                    )
                        ...
                        pd.DataFrame(
                            {
                                ...
                                "baseAssetAmountFilled": counterparty_df[
                                    "baseAssetAmountFilled"
                                ],
                                "quoteAssetAmountFilled": counterparty_df[
                                    "quoteAssetAmountFilled"
                                ],
                ...
                if not auth_taker_df.empty:
                    col11, col12 = st.columns(2)
                    auth_taker_df["execPrice"] = auth_taker_df.apply(
                        calc_taker_exec_price, axis=1
                    )
                    auth_taker_df["ts"] = pd.to_datetime(
                        auth_taker_df["ts"], unit="s", utc=True
                    )
                        ...
                        pd.DataFrame(
                            {
                                ...
                                "makerOrderDirection": auth_taker_df[
                                    "makerOrderDirection"
                                ],
                                "taker": auth_taker_df["taker"],
                                "takerOrderId": auth_taker_df["takerOrderId"],
                                "takerOrderDirection": auth_taker_df[
                                    "takerOrderDirection"
                                ],
                                "baseAssetAmountFilled": auth_taker_df[
                                    "baseAssetAmountFilled"
                                ],
                                "quoteAssetAmountFilled": auth_taker_df[
                                    "quoteAssetAmountFilled"
                                ],
    ...
    with tabs[3]:
        ...
        s1, s2 = st.columns(2)
        N = s1.number_input(
            "score given away each weekly period:",
            2e6,
            # 0.005/4 * 100
        )
        s1.write(
            "37.5-62.5 taker/maker. maker: 70/30 pct split between volume/liquidity score"
        )
        ...
        subtabs = st.tabs(
            ["overview", "per market alloc", "volume only", "liquidity only"]
        )
        dfs2 = dfs
        ...
        dfs2["volumeScore1"] = dfs2["quoteAssetAmountFilled"] * dfs2[
            "actionExplanation"
        ].apply(lambda x: {"orderFilledWithMatchJit": 10}.get(x, 1))
        dfs2["ticketScore1"] = dfs2["takerFee"] * dfs2.apply(
            get_ticket_multiplier, axis=1
        )
        market_groupings = (
            dfs.groupby(["marketType", "marketIndex"])[
                ["quoteAssetAmountFilled", "volumeScore1"]
            ]
            .sum()
        ...
        market_groupings["percentOfVolume"] = (
            market_groupings["quoteAssetAmountFilled"]
            / market_groupings["quoteAssetAmountFilled"].sum()
            * 100
        )
        ...
        market_groupings["ScorePercentage"] = (
            market_groupings["ScorePercentage"] / total_percentage * 100
        )
        ...
        mask_perp = market_groupings.index.get_level_values("marketType").str.contains(
            "perp"
        )
        market_groupings.loc[mask_perp, "ScorePercentage"] = market_groupings.loc[
            mask_perp, "ScorePercentage"
        ].clip(lower=perp_lower_bound)
        market_groupings.loc[~mask_perp, "ScorePercentage"] = market_groupings.loc[
            ~mask_perp, "ScorePercentage"
        ].clip(lower=other_lower_bound)
        attempts = 0
        while (
            market_groupings["ScorePercentage"].sum() - 100
        ) > 1e-2 and attempts < 15:
            ...
            market_groupings["ScorePercentage"] /= (
                market_groupings["ScorePercentage"].sum() * 0.01
            )
        ...
        dfs2["multiplied_score"] = dfs2.apply(
            lambda row: row["volumeScore1"]
            * (
                market_groupings.loc[
                    (row["marketType"], row["marketIndex"]), "ScorePercentage"
                ]
                / 100
            )
            * N_M
            ...
            * (
                1
                / market_groupings.loc[
                    (row["marketType"], row["marketIndex"]), "volumeScore1"
                ]
            ),
            axis=1,
        )
        ...
        with subtabs[2]:
            st.metric(
                "total maker volume score distributed",
                dfs2["multiplied_score"].sum(),
                "vs " + str(N * 0.7),
            )

            st.header("user scores")
            st.write(
                dfs2.groupby(["maker"])["multiplied_score"]
                .sum()
                .sort_values(ascending=False)
            )
            ...
            st.write(
                dfs2.groupby(["marketType", "marketIndex", "maker"])["multiplied_score"]
                .sum()
                .sort_values(ascending=False)
            )
            ...
            if dolpeval:
                ...
                lp_agg_result = (
                    lp_dfs[["name", "slot", "sqrtK", "lpShares"]]
                    .fillna(0)
                    .astype(int)
                    .groupby(["name", "slot"])
                    .agg({"sqrtK": "mean", "lpShares": "sum"})
                )

                lp_agg_result["user own %"] = (
                    lp_agg_result["lpShares"] / lp_agg_result["sqrtK"] * 100
                )
            ...
            if lp_result is not None:
                ...
                rrrr = dfs2[dfs2.maker == "vAMM"][
                    ["slot", "marketType", "marketIndex", "maker", "multiplied_score"]
                ]

                rrrr["marketIndex"] = rrrr["marketIndex"].astype(int)
                rrrr = (
                    rrrr.groupby(["slot", "marketIndex", "marketType", "maker"])
                    .sum()
                    .reset_index()
                )  # .set_index('slot')
                ...
                rrrr["lp_snap_slot"] = (
                    rrrr["slot"].apply(lambda x: get_most_recent_slot(x)).astype(int)
                )
                rrrr = (
                    rrrr.groupby(["marketIndex", "marketType", "maker", "lp_snap_slot"])
                    .sum()
                    .reset_index()
                    .drop(["slot"], axis=1)
                )
                ...
                merged_df = pd.merge_asof(
                    lp_result.sort_values("slot"),
                    rrrr.sort_values("lp_snap_slot"),
                    by="marketIndex",
                    left_on="slot",
                    right_on="lp_snap_slot",
                    direction="forward",
                )
                ...
                st.write(
                    merged_df["multiplied_score"].sum(), rrrr["multiplied_score"].sum()
                )

                merged_df["dlp_frac"] = (
                    merged_df["lpShares"] / merged_df["sqrtK"]
                ).apply(lambda x: min(0.4, x))
                merged_df["dlp_multiplied_score"] = (
                    merged_df["dlp_frac"] * merged_df["multiplied_score"]
                )

                # st.write(merged_df[merged_df.user==merged_df.user.unique()[0]])#.pivot_table(index='', column='user')
                # *merged_df['multiplied_score']
                dlp_scored = merged_df.groupby(
                    [
                        "user",
                        # 'marketIndex',
                        "marketType",
                    ]
                ).agg({"dlp_multiplied_score": "sum"})
                ...
                st.write(
                    "total dlp score by market:",
                    dlp_scored_by_market["dlp_multiplied_score"].sum(),
                )
        ...
        with subtabs[3]:
            liquidity_dfs, liqudity_data_urls = await load_liquidity_agg_score_async(
                market_names, True
            )
            ...
            total_score_by_market = liquidity_dfs.groupby(
            ...
            def get_mult_score(row):
                ...
                if idx in market_groupings.index:
                    t2 = (
                        t1
                        * (market_groupings.loc[idx, "ScorePercentage"] / 100)
                        * N_M
                        * 0.3
                    )
                    return t2
            ...
            ldf3 = (
                liquidity_dfs.groupby(["user", "market_type", "market_index"])[
                    ["score"]
                ]
                .sum()
                .reset_index()
            ...
            st.write(
                f"ensure it all sums to target {N_M * 0.3}",
                "vs",
                f"{ldf3['multiplied_score'].sum()}",
            )
        ...
        with subtabs[0]:
            if dolpeval:
                ...
                ums = pd.read_csv(url_user_authority_map, index_col=[0])
                ...
                # st.write('N', N)
                taker_score = (
                    dfs2.groupby(["taker"])["ticketScore1"].sum()
                    / dfs2["ticketScore1"].sum()
                ) * N_T
                ...
                fin = pd.concat(
                    [vol_comp, liq_comp, vol_comp_dlp, taker_score, volume_df]
                ).fillna(0)
                st.write(fin)
                ...
                st.metric(
                    "maker_score score distributed", f"{fin['maker_score'].sum()}"
                )
                ...
                fin = fin.merge(
                    ums[["userAccount", "authority"]],
                    left_on="user",
                    right_on="userAccount",
                    how="left",
                )
