...
from anchorpy import Context, EventParser, Idl, Program, Provider, Wallet
...
from driftpy.drift_user import UserAccount, get_token_amount
...
async def vaults(ch: DriftClient, env):
    ...
    with tabs[0]:
        ...
        s1.metric(
            "number of vaults:",
            len(all_vaults),
            str(len(all_vault_deps)) + " vault depositor(s)",
        )
        chu = None
        if len(vault_df):
            user_keys = (
                vault_df.astype(str)
                .T.sort_values(by="init_ts", ascending=False)
                .T.loc["user"]
                .values
            )
            ...
            for i, vault_user in enumerate(vault_users):
                ...
                with st.expander(f"> {nom}: ${equity:,.2f}/{max_tokens:,.2f}"):
                    st.markdown(
                        f"balance: `${equity:,.2f}` [[web ui](https://app.drift.trade/?authority={vault_user.authority})]"
                    )

                    prof_share = float(dat.loc["profit_share"]) / 1e4
                    mang_share = float(dat.loc["management_fee"]) / 1e4
                    st.write(
                        f"fee structure: `{mang_share:,.2f}% / {prof_share:,.2f}%` (management / profit fee)"
                    )
                    if float(dat.loc["total_shares"]) != 0:
                        user_owned_frac = float(dat.loc["user_shares"]) / float(
                            dat.loc["total_shares"]
                        )
                        st.write(f"user-owned: `${equity * user_owned_frac:,.2f}`")

                        this_vault_deps = (
                            vault_dep_def.T[
                                vault_dep_def.T["vault"].astype(str)
                                == str(vault_user.authority)
                            ]
                            .sort_values("vault_shares", ascending=False)
                            .T
                        )
                        ...
                        for j in range(len(this_vault_deps.columns)):
                            ...
                            tuser_owned_frac = float(
                                tv_dep.loc["vault_shares"]
                            ) / float(dat.loc["total_shares"])
                            tcur_val = equity * tuser_owned_frac
                            tpnl = tcur_val - float(tv_dep.loc["net_deposits"]) / 1e6
                            tprof_fees = (
                                float(tv_dep.loc["cumulative_profit_share_amount"])
                                / 1e6
                                * prof_share
                                / 1e2
                            )
                            st.write(
                                f"> {tv_nom}-owned: `${tcur_val:,.3f}` | pnl = `${tpnl:,.3f}` | u.b. profit fees: `${tprof_fees:,.3f}`"
                            )

                        st.write(
                            f"manager-owned: `${equity - equity * user_owned_frac:,.2f}` "
                        )
